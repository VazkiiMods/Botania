...
public class TileManaBeacon extends TileMod {
	...
	public void updateEntity() {
		...
		if(!redstone) {
			...
			for(int i = 0; i < 3; i++)
				Botania.proxy.wispFX(worldObj, xCoord + 0.5, yCoord, zCoord + 0.5, color[0], color[1], color[2], (float) Math.random() * 5 + 1F, (float) (Math.random() - 0.5F), 10F * (float) Math.sqrt((256F / (256F - (float) yCoord))), (float) (Math.random() - 0.5F));
