...
public class ItemItemFinder extends ItemBauble {
	...
	public void tickClient(ItemStack stack, EntityPlayer player) {
		...
		for(String token : tokens) {
			...
			if(token.contains(",")) {
				...
				if(e != null)
					if(Math.random() < 0.6)
						Botania.proxy.wispFX(player.worldObj, e.posX + (float) (Math.random() * 0.5 - 0.25) * 0.45F, e.posY + 0.2, e.posZ + (float) (Math.random() * 0.5 - 0.25) * 0.45F, 0.2F * (float) Math.random(), 1F, 0.2F * (float) Math.random(), 0.15F + 0.05F * (float) Math.random(), -0.05F - 0.03F * (float) Math.random());
			}
		}
	}
	...
	public void tickServer(ItemStack stack, EntityPlayer player) {
		...
		if(pstack != null || player.isSneaking()) {
			...
			List<EntityItem> items = player.worldObj.getEntitiesWithinAABB(EntityItem.class, AxisAlignedBB.getBoundingBox(player.posX - range, player.posY - range, player.posZ - range, player.posX + range, player.posY + range, player.posZ + range));
