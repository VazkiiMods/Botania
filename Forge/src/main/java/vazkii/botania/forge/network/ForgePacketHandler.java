...
public class ForgePacketHandler {
	...
	public static void init() {
		...
		CHANNEL.registerMessage(i++, IndexKeybindRequestPacket.class, IndexKeybindRequestPacket::encode,
			IndexKeybindRequestPacket::decode,
			makeServerBoundHandler(IndexKeybindRequestPacket::handle));
		CHANNEL.registerMessage(i++, IndexRequestPacket.class, IndexRequestPacket::encode, IndexRequestPacket::decode,
			makeServerBoundHandler(IndexRequestPacket::handle));
		...
		// Clientbound
		CHANNEL.registerMessage(i++, AvatarSkiesRodPacket.class, AvatarSkiesRodPacket::encode,
			AvatarSkiesRodPacket::decode,
			makeClientBoundHandler(AvatarSkiesRodPacket.Handler::handle));
		CHANNEL.registerMessage(i++, BotaniaEffectPacket.class, BotaniaEffectPacket::encode,
		...
		CHANNEL.registerMessage(i++, SpawnGaiaGuardianPacket.class, SpawnGaiaGuardianPacket::encode,
			SpawnGaiaGuardianPacket::decode,
			makeClientBoundHandler(SpawnGaiaGuardianPacket.Handler::handle));
		CHANNEL.registerMessage(i++, UpdateItemsRemainingPacket.class, UpdateItemsRemainingPacket::encode,
			UpdateItemsRemainingPacket::decode,
			makeClientBoundHandler(UpdateItemsRemainingPacket.Handler::handle));
	}

	private static <T> BiConsumer<T, Supplier<NetworkEvent.Context>> makeServerBoundHandler(TriConsumer<T,
		MinecraftServer, ServerPlayer> handler) {
		return (m, ctx) -> {
