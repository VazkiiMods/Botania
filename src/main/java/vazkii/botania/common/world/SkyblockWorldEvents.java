...
public final class SkyblockWorldEvents {
        ...
        if (WorldTypeSkyblock.isWorldSkyblock(event.world)) {
            ...
            if (event.action == Action.RIGHT_CLICK_BLOCK && equipped == null && event.entityPlayer.isSneaking()) {
                ...
                if (block == Blocks.grass || block == Blocks.dirt) {
                    if (event.world.isRemote) event.entityPlayer.swingItem();
                    else {
                        event.world.playSoundEffect(
                                ...
                                block.stepSound.getVolume() * 0.4F,
                                block.stepSound.getPitch() + (float) (Math.random() * 0.2 - 0.1));
                        if (Math.random() < 0.8)
                            event.entityPlayer.dropPlayerItemWithRandomChoice(
                                    new ItemStack(ModItems.manaResource, 1, 21), false);
                    }
                }
            } else if (equipped != null
                    && equipped.getItem() == Items.bowl
                    && event.action == Action.RIGHT_CLICK_BLOCK
                    && !event.world.isRemote) {
                MovingObjectPosition movingobjectposition =
                        ToolCommons.raytraceFromEntity(event.world, event.entityPlayer, true, 4.5F);
                        ...
                        if (event.world.getBlock(i, j, k).getMaterial() == Material.water) {
                            ...
                            if (equipped.stackSize <= 0)
                                event.entityPlayer.inventory.setInventorySlotContents(
                                        event.entityPlayer.inventory.currentItem, new ItemStack(ModItems.waterBowl));
                            else
                                event.entityPlayer.dropPlayerItemWithRandomChoice(
                                        new ItemStack(ModItems.waterBowl), false);
                        }
                    }
                }
            }
    ...
    public void onDrops(HarvestDropsEvent event) {
        if (WorldTypeSkyblock.isWorldSkyblock(event.world) && event.block == Blocks.tallgrass) {
            ...
            if (stackToRemove != null) {
                ...
                event.drops.add(
                        new ItemStack(event.world.rand.nextBoolean() ? Items.pumpkin_seeds : Items.melon_seeds));
            }
        }
    }
    ...
    public static void createSkyblock(World world, int x, int y, int z) {
        ...
        ((TileManaFlame) world.getTileEntity(x + 1, y + 2, z + 1))
                .setColor(new Color(
                                70 + world.rand.nextInt(185),
                                70 + world.rand.nextInt(185),
                                70 + world.rand.nextInt(185))
                        .getRGB());
        ...
        for (int[] root : rootPositions) world.setBlock(x + root[0], y + root[1], z + root[2], ModBlocks.root);
