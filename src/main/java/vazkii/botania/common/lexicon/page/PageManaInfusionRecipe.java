...
public class PageManaInfusionRecipe extends PageRecipe {
    ...
    private static final ResourceLocation manaInfusionOverlay =
            new ResourceLocation(LibResources.GUI_MANA_INFUSION_OVERLAY);
    ...
    public void onPageAdded(LexiconEntry entry, int index) {
        for (RecipeManaInfusion recipe : recipes)
            if (recipe != null) LexiconRecipeMappings.map(recipe.getOutput(), entry, index);
    }
    ...
    public void renderRecipe(IGuiLexiconEntry gui, int mx, int my) {
        ...
        renderItemAtGridPos(
                ...
                new ItemStack(
                        ModBlocks.pool,
                        1,
                        recipe.getOutput().getItem() == Item.getItemFromBlock(ModBlocks.pool) ? 2 : 0),
                false);

        renderItemAtGridPos(gui, 3, 1, recipe.getOutput(), false);

        if (recipe.isAlchemy()) renderItemAtGridPos(gui, 1, 2, new ItemStack(ModBlocks.alchemyCatalyst), false);
        ...
        if (mx > x + 1 && mx <= x + 101 && my > y - 14 && my <= y + 11) ratio = 1;
        ...
        if (mx > dx && mx <= dx + dw && my > dy && my <= dy + 10) hoveringOverDrop = true;
        ...
        if (hoveringOverDrop) {
            ...
            String tip0 = StatCollector.translateToLocal("botaniamisc.dropTip0")
                    .replaceAll("%key%", EnumChatFormatting.GREEN + key + EnumChatFormatting.WHITE);
            String tip1 = StatCollector.translateToLocal("botaniamisc.dropTip1")
                    .replaceAll("%key%", EnumChatFormatting.GREEN + key + EnumChatFormatting.WHITE);
    ...
    public List<ItemStack> getDisplayedRecipes() {
        ...
        for (RecipeManaInfusion r : recipes) list.add(r.getOutput());
