...
public class PagePetalRecipe<T extends RecipePetals> extends PageRecipe {
    ...
    public void onPageAdded(LexiconEntry entry, int index) {
        for (T recipe : recipes) if (recipe != null) LexiconRecipeMappings.map(recipe.getOutput(), entry, index);
    }
    ...
    public void renderRecipe(IGuiLexiconEntry gui, int mx, int my) {
        ...
        float currentDegree = ConfigHandler.lexiconRotatingItems
                ? GuiScreen.isShiftKeyDown() ? ticksElapsed : (float) (ticksElapsed + ClientTickHandler.partialTicks)
                : 0;
