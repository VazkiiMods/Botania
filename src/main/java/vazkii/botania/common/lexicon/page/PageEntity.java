...
public class PageEntity extends LexiconPage {
    ...
    public int getEntityScale(int targetSize) {
        ...
        if (dummyEntity.width < dummyEntity.height) entity_size = dummyEntity.height;
    ...
    public void renderEntity(IGuiLexiconEntry gui, Entity entity, int x, int y, int scale, float rotation) {
        ...
        if (relativeMouseX >= x - dummyEntity.width * scale / 2 - 10
                ...
                && relativeMouseX <= x + dummyEntity.width * scale / 2 + 10
                && relativeMouseY <= y + 20) tooltipEntity = true;
    }
