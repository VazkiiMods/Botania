...
public class EntityPixie extends EntityFlyingCreature {
    ...
    protected void updateEntityActionState() {
        ...
        if (target != null) {
            ...
            if (Math.sqrt(d3) < 1F) {
                ...
                if (effect != null && !(target instanceof EntityPlayer)) target.addPotionEffect(effect);
                die();
            }
        }
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
        if (getType() == 0 && par1DamageSource.getEntity() != summoner
                || getType() == 1 && par1DamageSource.getEntity() instanceof EntityPlayer)
    ...
    public void onEntityUpdate() {
        ...
        if (getAttackTarget() == null || ticksExisted > 200) die();
        ...
        if (worldObj.isRemote)
            for (int i = 0; i < 4; i++)
                Botania.proxy.sparkleFX(
                        ...
                        posY + 0.5 + (Math.random() - 0.5) * 0.25,
                        posZ + (Math.random() - 0.5) * 0.25,
                        dark ? 0.1F : 1F,
                        dark ? 0.025F : 0.25F,
                        dark ? 0.09F : 0.9F,
