    ...
    public void onUpdate() {
        ...
        if (bounces >= MAX_BOUNCES || ticksExisted > 60) {
            ...
            if (thrower == null) dropAndKill();
                ...
                Vector3 motion = Vector3.fromEntityCenter(thrower)
                        .sub(Vector3.fromEntityCenter(this))
                        .normalize();
                motionX = motion.x;
                motionY = motion.y;
                motionZ = motion.z;
                if (MathHelper.pointDistanceSpace(posX, posY, posZ, thrower.posX, thrower.posY, thrower.posZ) < 1)
                    if (!(thrower instanceof EntityPlayer
                            && (((EntityPlayer) thrower).capabilities.isCreativeMode
                                    || ((EntityPlayer) thrower).inventory.addItemStackToInventory(getItemStack()))))
                        dropAndKill();
        ...
        if (pos.entityHit != null && pos.entityHit instanceof EntityLivingBase && pos.entityHit != thrower) {
            ((EntityLivingBase) pos.entityHit)
                    .attackEntityFrom(
                            ...
                            12);
            if (fire) ((EntityLivingBase) pos.entityHit).setFire(5);
            ...
            if (bounces < MAX_BOUNCES) {
                ...
                Vector3 movementVec = normalVector
