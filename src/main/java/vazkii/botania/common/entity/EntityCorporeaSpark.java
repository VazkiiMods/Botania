...
public class EntityCorporeaSpark extends Entity implements ICorporeaSpark {
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote ? firstUpdateClient : firstUpdateServer) {
            if (isMaster()) restartNetwork();
            else findNetwork();

            if (worldObj.isRemote) firstUpdateClient = false;
            else firstUpdateServer = false;
        }

        if (master != null && (((Entity) master).isDead || master.getNetwork() != getNetwork())) master = null;

        int displayTicks = getItemDisplayTicks();
        if (displayTicks > 0) setItemDisplayTicks(displayTicks - 1);
        else if (displayTicks < 0) setItemDisplayTicks(displayTicks + 1);
    }

    @Override
    public void setDead() {
        super.setDead();
        if (!worldObj.isRemote) entityDropItem(new ItemStack(ModItems.corporeaSpark, 1, isMaster() ? 1 : 0), 0F);
    ...
    public void registerConnections(ICorporeaSpark master, ICorporeaSpark referrer, List<ICorporeaSpark> connections) {
        ...
        for (ICorporeaSpark spark : sparks) {
            if (spark == null
                    || connections.contains(spark)
                    || spark.getNetwork() != getNetwork()
                    || spark.isMaster()
                    || ((Entity) spark).isDead) continue;
        ...
        this.master = master;
        if (worldObj.isRemote) connectionsClient = connections;
    ...
    void restartNetwork() {
        if (worldObj.isRemote) connectionsClient = new ArrayList();
    ...
    void displayRelatives(ArrayList<ICorporeaSpark> checked, ICorporeaSpark spark) {
        ...
        if (sparks.isEmpty()) EntitySpark.particleBeam((Entity) spark, (Entity) spark.getMaster());
        else
