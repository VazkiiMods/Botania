...
public class EntityManaBurst extends EntityThrowable implements IManaBurst {
    ...
    public EntityManaBurst(World world) {
        ...
        for (int i = 0; i < dataWatcherEntries; i++) {
            ...
            if (i == 4 || i == 5) dataWatcher.addObject(j, 0F);
    ...
    public EntityManaBurst(IManaSpreader spreader, boolean fake) {
        ...
        float f = 0.4F;
        double mx = MathHelper.sin(rotationYaw / 180.0F * (float) Math.PI)
                * MathHelper.cos(rotationPitch / 180.0F * (float) Math.PI)
                * f
                / 2D;
        double mz = -(MathHelper.cos(rotationYaw / 180.0F * (float) Math.PI)
                        * MathHelper.cos(rotationPitch / 180.0F * (float) Math.PI)
                        * f)
                / 2D;
        ...
        if (!Double.isNaN(par1Vec3.xCoord) && !Double.isNaN(par1Vec3.yCoord) && !Double.isNaN(par1Vec3.zCoord)) {
            if (!Double.isNaN(par2Vec3.xCoord) && !Double.isNaN(par2Vec3.yCoord) && !Double.isNaN(par2Vec3.zCoord)) {
                ...
                if (block != null
                        && (!par4
                                || block == null
                                || block.getCollisionBoundingBoxFromPool(worldObj, l, i1, j1) != null)
                        && block != Blocks.air
                        && block.canCollideCheck(l1, par3)) {
                    MovingObjectPosition movingobjectposition =
                            block.collisionRayTrace(worldObj, l, i1, j1, par1Vec3, par2Vec3);
                ...
                while (k1-- >= 0) {
                    ...
                    if ((!par4 || block1 == null || block1.getCollisionBoundingBoxFromPool(worldObj, l, i1, j1) != null)
                            && block1 != Blocks.air
                            && block1.canCollideCheck(j2, par3)) {
                        MovingObjectPosition movingobjectposition1 =
                                block1.collisionRayTrace(worldObj, l, i1, j1, par1Vec3, par2Vec3);
    ...
    public void writeEntityToNBT(NBTTagCompound par1nbtTagCompound) {
        ...
        if (stack != null) stack.writeToNBT(lensCmp);
    ...
    public void readEntityFromNBT(NBTTagCompound par1nbtTagCompound) {
        ...
        if (stack != null) setSourceLens(stack);
        ...
        if (hasShooter) {
            ...
            if (identity == null
                    ...
                    || least != identity.getLeastSignificantBits()) shooterIdentity = new UUID(most, least);
        }
    }
    ...
    public void particles() {
        ...
        if (fake) {
            ...
            if (monocle) Botania.proxy.setWispFXDepthTest(false);
            ...
            if (ConfigHandler.subtlePowerSystem)
                ...
                do {
                    ...
                    size = osize
                            + ((float) Math.random() - 0.5F) * 0.065F
                            + (float) Math.sin(new Random(entityUniqueID.getMostSignificantBits()).nextInt(9001))
                                    * 0.4F;
                    Botania.proxy.wispFX(
                            worldObj,
                            posX,
                    ...
                    if (getEntityData().hasKey(ItemTinyPlanet.TAG_ORBIT)) break;
            ...
            if (monocle) Botania.proxy.setWispFXDepthTest(true);
        }
    }
    ...
    protected void onImpact(MovingObjectPosition movingobjectposition) {
        ...
        if (movingobjectposition.entityHit == null) {
            ...
            if (tile instanceof IManaCollisionGhost
                            && ((IManaCollisionGhost) tile).isGhost()
                            && !(block instanceof IManaTrigger)
                    || block instanceof BlockBush
                    || block instanceof BlockLeaves) return;

            if (BotaniaAPI.internalHandler.isBuildcraftPipe(tile)) return;

            ChunkCoordinates coords = getBurstSourceChunkCoordinates();
            if (tile != null
                    && (tile.xCoord != coords.posX || tile.yCoord != coords.posY || tile.zCoord != coords.posZ))
                collidedTile = tile;
            ...
            if (tile == null
                ...
                if (!fake
                        && !noParticles
                        && (!worldObj.isRemote || tile instanceof IClientManaHandler)
                        && tile != null
        ...
        ILensEffect lens = getLensInstance();
        if (lens != null)
            dead = lens.collideBurst(
                    this,
                    movingobjectposition,
                    collidedTile != null
                            && collidedTile instanceof IManaReceiver
                            && ((IManaReceiver) collidedTile).canRecieveManaFromBursts(),
                    dead,
                    getSourceLens());
        ...
        if (collided
                && !hasAlreadyCollidedAt(
                        movingobjectposition.blockX, movingobjectposition.blockY, movingobjectposition.blockZ))
            alreadyCollidedAt.add(getCollisionLocString(
                    movingobjectposition.blockX, movingobjectposition.blockY, movingobjectposition.blockZ));
    ...
    protected void onRecieverImpact(IManaReceiver tile, int x, int y, int z) {
        ...
        if (tile instanceof IThrottledPacket) ((IThrottledPacket) tile).markDispatchable();
    ...
    public void setDead() {
        ...
        if (!fake) {
            ...
            if (tile != null && tile instanceof IManaSpreader) ((IManaSpreader) tile).setCanShoot(true);
    ...
    public void setCollidedAt(int x, int y, int z) {
        if (!hasAlreadyCollidedAt(x, y, z)) alreadyCollidedAt.add(getCollisionLocString(x, y, z));
    }
    ...
    public void ping() {
        TileEntity tile = getShooter();
        if (tile != null && tile instanceof IPingable) ((IPingable) tile).pingback(this, getShooterUIID());
    }
    ...
    public boolean shouldDoFakeParticles() {
        ...
        if (tile != null && tile instanceof IManaSpreader)
            return getMana() != getStartingMana() && fullManaLastTick
    ...
    public void incrementFakeParticleTick() {
        ...
        if (tile != null && tile instanceof IManaSpreader) {
            ...
            if (spreader.getLastBurstDeathTick() != -1
                    && spreader.getBurstParticleTick() > spreader.getLastBurstDeathTick())
