...
public class EntityFlameRing extends Entity {
    ...
    public void onEntityUpdate() {
        ...
        for (int i = 0; i < Math.min(90, ticksExisted); i++) {
            ...
            if (worldObj.rand.nextInt(ticksExisted < 90 ? 8 : 20) == 0) {
                ...
                Botania.proxy.wispFX(
                        ...
                        (float) Math.random() * 0.25F,
                        0.65F + (float) Math.random() * 0.45F,
                        (float) (Math.random() - 0.5F) * 0.15F,
                        0.055F + (float) Math.random() * 0.025F,
                        (float) (Math.random() - 0.5F) * 0.15F);
                ...
                Botania.proxy.wispFX(
                        ...
                        gs,
                        0.65F + (float) Math.random() * 0.45F,
                        -0.155F - (float) Math.random() * 0.025F);
            }
        }

        if (worldObj.rand.nextInt(20) == 0) worldObj.playSoundAtEntity(this, "fire.fire", 1F, 1F);
        ...
        if (ticksExisted > 45) {
            AxisAlignedBB boundingBox = AxisAlignedBB.getBoundingBox(posX, posY, posZ, posX, posY, posZ)
