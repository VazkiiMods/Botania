...
public class BlockPistonRelay extends BlockMod implements IWandable, ILexiconable {
    ...
    public static class WorldData extends WorldSavedData {
        ...
        public void writeToNBT(NBTTagCompound nbttagcompound) {
            for (String s : mappedPositions.keySet()) nbttagcompound.setString(s, mappedPositions.get(s));
        }
    ...
    public void tickEnd(TickEvent event) {
                    ...
                    if (server != null && getTimeInCoords(s) == 0) {
...

                        {
                            ...
                            if (world.isAirBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ))
                                world.setBlock(
                                        x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, ModBlocks.pistonRelay);
                            else if (!world.isRemote) {
                                ItemStack stack = new ItemStack(ModBlocks.pistonRelay);
                                world.spawnEntityInWorld(new EntityItem(
                                        world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, stack));
                            }
                            checkedCoords.add(s);
                            newPos = getCoordsAsString(
                                    world.provider.dimensionId, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ);
                        }
                        ...
                        if (mappedPositions.containsKey(s)) {
                            ...
                            if (!sticky
                                    && tile == null
                                    && mat.getMaterialMobility() == 0
                                    && srcBlock.getBlockHardness(world, x, y, z) != -1
                                    && !srcBlock.isAir(world, x, y, z)) {
                                Material destMat = world.getBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ)
                                        .getMaterial();
                                if (world.isAirBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ)
                                        || destMat.isReplaceable()) {
        ...
        for (String s : remove) {
            ...
            if (checkedCoords.contains(s)) checkedCoords.remove(s);
        }
        removeThese.clear();
    }
