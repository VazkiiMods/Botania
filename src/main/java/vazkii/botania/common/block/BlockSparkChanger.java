...
public class BlockSparkChanger extends BlockModContainer implements ILexiconable {
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        } else if (!power && powered) world.setBlockMetadataWithNotify(x, y, z, meta & -9, 4);
    }

    @Override
    public boolean onBlockActivated(
            World world, int x, int y, int z, EntityPlayer player, int s, float xs, float ys, float zs) {
        ...
        if (cstack != null) {
            ...
            changer.markDirty();
            if (!player.inventory.addItemStackToInventory(cstack)) player.dropPlayerItemWithRandomChoice(cstack, false);
            return true;
            ...
            pstack.stackSize--;
            if (pstack.stackSize == 0) player.inventory.setInventorySlotContents(player.inventory.currentItem, null);
