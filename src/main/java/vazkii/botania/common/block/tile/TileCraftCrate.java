...
public class TileCraftCrate extends TileOpenCrate {
    ...
    public void updateEntity() {
        if (!worldObj.isRemote && (craft(true) && canEject() || isFull())) ejectAll();

        int newSignal = 0;
        for (; newSignal < 9; newSignal++) // dis for loop be derpy
        if (!isLocked(newSignal) && getStackInSlot(newSignal) == null) break;
    ...
    boolean craft(boolean fullCheck) {
        ...
        InventoryCrafting craft = new InventoryCrafting(
                new Container() {
                    @Override
