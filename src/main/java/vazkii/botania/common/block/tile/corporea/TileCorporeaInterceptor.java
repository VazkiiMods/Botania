    ...
    public void interceptRequestLast(
        ...
        for (ItemStack stack : filter)
            if (requestMatches(request, stack)) {
                int missing = count;
                for (ItemStack stack_ : stacks) missing -= stack_.stackSize;
                ...
                if (missing > 0 && getBlockMetadata() == 0) {
                    ...
                    for (ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS) {
                        ...
                        if (tile != null && tile instanceof TileCorporeaRetainer)
                            ((TileCorporeaRetainer) tile)
                                    .setPendingRequest(
                                            requestor.xCoord, requestor.yCoord, requestor.zCoord, request, count);
                    }
