...
public class TileLightRelay extends TileMod implements IWandBindable {
    ...
    public void mountEntity(Entity e) {
        if (e.ridingEntity != null || worldObj.isRemote || bindY == -1 || !isValidBinding()) return;
        ...
        if (!(e instanceof EntityItem)) {
            ...
            if (e instanceof EntityPlayer) ((EntityPlayer) e).addStat(ModAchievements.luminizerRide, 1);
        }
    }
    ...
    public void updateEntity() {
        ...
        if (bindY > -1 && isValidBinding()) {
            ...
            for (int i = start; i < start + count; i++) {
                ...
                Vector3 vecRot = vecMag.copy()
                        .crossProduct(Vector3.one)
                        .multiply(mul)
                        .rotate(rad, vecMag)
                        .add(vecTip);
            ...
            if (endpoint != null && !worldObj.isRemote) {
                float range = 0.5F;
                List<EntityEnderPearl> enderPearls = worldObj.getEntitiesWithinAABB(
    ...
    public ChunkCoordinates getEndpoint() {
        ...
        boolean run = true;
        while (run) {
            if (pointsPassed.contains(relay)) return null; // Circular path
            pointsPassed.add(relay);

            ChunkCoordinates coords = relay.getBinding();
            if (coords == null) return lastCoords;

            TileEntity tile = worldObj.getTileEntity(coords.posX, coords.posY, coords.posZ);
            if (tile != null && tile instanceof TileLightRelay) relay = (TileLightRelay) tile;
    ...
    public boolean bindTo(EntityPlayer player, ItemStack wand, int x, int y, int z, int side) {
        if (player.worldObj.getBlock(x, y, z) != ModBlocks.lightRelay
                || MathHelper.pointDistanceSpace(x, y, z, xCoord, yCoord, zCoord) > MAX_DIST) return false;
    ...
    public static class EntityPlayerMover extends Entity {
        ...
        public void onUpdate() {
            ...
            if (x == exitX && y == exitY && z == exitZ) {
                ...
                if (tile != null && tile instanceof TileLightRelay) {
                    ...
                    if (meta > 0) {
                        ...
                        worldObj.scheduleBlockUpdate(
                                ...
                                z,
                                tile.getBlockType(),
                                tile.getBlockType().tickRate(worldObj));
                    }
                ...
                Vector3 motVec = thisVec.negate()
                        .add(exitX + 0.5, exitY + 0.5, exitZ + 0.5)
                        .normalize()
                        .multiply(0.5);
