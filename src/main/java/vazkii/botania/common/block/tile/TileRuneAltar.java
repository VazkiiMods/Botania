...
public class TileRuneAltar extends TileSimpleInventory implements ISidedInventory, IManaReceiver {
    ...
    public boolean addItem(EntityPlayer player, ItemStack stack) {
        if (cooldown > 0 || stack.getItem() == ModItems.twigWand || stack.getItem() == ModItems.lexicon) return false;
        ...
        if (stack.getItem() == Item.getItemFromBlock(ModBlocks.livingrock) && stack.getItemDamage() == 0) {
            ...
            EntityItem item = new EntityItem(
                    worldObj, xCoord + 0.5, yCoord + 1, zCoord + 0.5, new ItemStack(ModBlocks.livingrock));
        ...
        if (did) VanillaPacketDispatcher.dispatchTEToNearbyPlayers(worldObj, xCoord, yCoord, zCoord);
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && manaToGet == 0) {
            ...
            for (EntityItem item : items)
                if (!item.isDead
                        ...
                        && item.getEntityItem().getItem() != Item.getItemFromBlock(ModBlocks.livingrock)) {
                    ItemStack stack = item.getEntityItem();
                    if (addItem(null, stack) && stack.stackSize == 0) item.setDead();
                }
        }
        ...
        if (recipeKeepTicks > 0) --recipeKeepTicks;
    ...
    public void updateRecipe() {
        ...
        getMana:
        {
            if (currentRecipe != null) this.manaToGet = currentRecipe.getManaUsage();
            else {
    ...
    public void trySetLastRecipe(EntityPlayer player) {
        ...
        if (!isEmpty()) VanillaPacketDispatcher.dispatchTEToNearbyPlayers(worldObj, xCoord, yCoord, zCoord);
    }

    public boolean hasValidRecipe() {
        for (RecipeRuneAltar recipe : BotaniaAPI.runeAltarRecipes) if (recipe.matches(this)) return true;
    ...
    public void onWanded(EntityPlayer player, ItemStack wand) {
        ...
        if (currentRecipe != null) recipe = currentRecipe;
        else
                        ...
                        if (stack != null) {
                            if (stack.getItem() == ModItems.rune
                                ...
                                EntityItem outputItem = new EntityItem(
                                        worldObj, xCoord + 0.5, yCoord + 1.5, zCoord + 0.5, stack.copy());
                                worldObj.spawnEntityInWorld(outputItem);
                            }
    ...
    public int[] getAccessibleSlotsFromSide(int var1) {
        ...
        for (int i = 0; i < getSizeInventory(); i++) if (getStackInSlot(i) != null) accessibleSlot = i;
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        ...
        int amt = 0;
        for (int i = 0; i < getSizeInventory(); i++) {
            if (getStackInSlot(i) == null) break;
            amt++;
        }
        ...
        if (amt > 0) {
            ...
            for (RecipeRuneAltar recipe : BotaniaAPI.runeAltarRecipes)
                if (recipe.matches(this)) {
                    ...
                    if (progress == 1F) mc.fontRenderer.drawStringWithShadow("+", xc + radius + 14, yc + 12, 0xFFFFFF);
                }
