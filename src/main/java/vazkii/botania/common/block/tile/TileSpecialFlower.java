...
public class TileSpecialFlower extends TileMod implements IWandBindable, ISubTileSlowableContainer {
    ...
    public void writeCustomNBT(NBTTagCompound cmp) {
        ...
        if (subTile != null) subTile.writeToPacketNBTInternal(subCmp);
    }
    ...
    public void readCustomNBT(NBTTagCompound cmp) {
        ...
        if (subTile == null
                || !BotaniaAPI.getSubTileStringMapping(subTile.getClass()).equals(subTileName))
            provideSubTile(subTileName);

        if (subTile != null) subTile.readFromPacketNBTInternal(subCmp);
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
        if (subTile != null) subTile.onBlockPlacedBy(world, x, y, z, entity, stack);
    }
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        if (subTile != null) subTile.onBlockAdded(world, x, y, z);
    }

    public void onBlockHarvested(World world, int x, int y, int z, int side, EntityPlayer player) {
        if (subTile != null) subTile.onBlockHarvested(world, x, y, z, side, player);
    }

    public ArrayList<ItemStack> getDrops(ArrayList<ItemStack> list) {
        if (subTile != null) subTile.getDrops(list);
    ...
    public int getSlowdownFactor() {
        ...
        if (below == Blocks.mycelium) return SLOWDOWN_FACTOR_MYCEL;

        if (below == Blocks.dirt) {
            int meta = worldObj.getBlockMetadata(xCoord, yCoord - 1, zCoord);
            if (meta == 2) return SLOWDOWN_FACTOR_PODZOL;
        }
