...
public class TilePump extends TileMod {
    ...
    public void updateEntity() {
            ...
            if (innerRingPos >= max) {
                ...
                for (int x = 0; x < 2; x++)
                    worldObj.spawnParticle(
                            "explode", xCoord + Math.random(), yCoord + Math.random(), zCoord + Math.random(), 0, 0, 0);
            }
        ...
        if (!hasCartOnTop) comparator = 0;
        if (!hasCart && active) setActive(false);
        if (active && hasRedstone) setActive(false);
    ...
    public void setActive(boolean active) {
        if (!worldObj.isRemote) {
            ...
            this.active = active;
            if (diff) VanillaPacketDispatcher.dispatchTEToNearbyPlayers(worldObj, xCoord, yCoord, zCoord);
        }
    }
}
