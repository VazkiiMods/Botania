...
public class TileSpreader extends TileSimpleInventory
        implements IManaCollector, IWandBindable, IKeyLocked, IThrottledPacket, IManaSpreader, IRedirectable {
    ...
    public void updateEntity() {
            ...
            if (tileAt instanceof IManaPool) {
                ...
                if (wasInNetwork && (pool != receiver || isRedstone())) {
                    if (pool instanceof IKeyLocked
                            && !((IKeyLocked) pool).getOutputKey().equals(getInputKey())) continue;
        ...
        if (!canShootBurst)
            if (pingbackTicks <= 0) {
                ...
                AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(x, y, z, x, y, z)
                        .expand(
        ...
        if (isredstone) shouldShoot = redstone && !redstoneLastTick;
    ...
    public void readCustomNBT(NBTTagCompound cmp) {
        ...
        if (cmp.getBoolean(TAG_HAS_IDENTITY)) {
            ...
            if (identity == null
                    || most != identity.getMostSignificantBits()
                    || least != identity.getLeastSignificantBits()) identity = new UUID(most, least);
        } else getIdentifier();
        ...
        if (cmp.hasKey(TAG_CAN_SHOOT_BURST)) canShootBurst = cmp.getBoolean(TAG_CAN_SHOOT_BURST);
        ...
        if (requestsClientUpdate && worldObj != null) {
            ...
            if (y != -1) {
                ...
                if (tile instanceof IManaReceiver) receiver = (IManaReceiver) tile;
                else receiver = null;
            } else receiver = null;
        }

        if (worldObj != null && worldObj.isRemote) hasReceivedInitialPacket = true;
    }
        ...
        if (!player.isSneaking()) {
            ...
            if (pos != null && pos.hitVec != null && !worldObj.isRemote) {
                ...
                if (pos.sideHit != 0 && pos.sideHit != 1) {
                    ...
                    double angle = Math.acos(clickVector.dotProduct(relative) / (relative.mag() * clickVector.mag()))
                            * 180D
                            / Math.PI;

                    rotationX = (float) angle + 180F;
                    if (clickVector.z < 0) rotationX = 360 - rotationX;
                }
    ...
    private boolean needsNewBurstSimulation() {
        if (worldObj.isRemote && !hasReceivedInitialPacket) return false;

        if (lastTentativeBurst == null) return true;
    ...
    public void checkForReceiver() {
        ...
        if (control != null && !control.allowBurstShooting(stack, this, false)) return;

        EntityManaBurst fakeBurst = getBurst(true);
        fakeBurst.setScanBeam();
        TileEntity receiver = fakeBurst.getCollidedTile(true);

        if (receiver != null && receiver instanceof IManaReceiver) this.receiver = (IManaReceiver) receiver;
    ...
    public EntityManaBurst getBurst(boolean fake) {
        ...
        BurstProperties props =
                new BurstProperties(maxMana, ticksBeforeManaLoss, manaLossPerTick, gravity, motionModifier, color);
        ...
        if (getCurrentMana() >= props.maxMana || fake) {
            if (mapmakerOverride) {
                ...
                burst.setMotion(
                        burst.motionX * mmForcedVelocityMultiplier,
                        burst.motionY * mmForcedVelocityMultiplier,
                        burst.motionZ * mmForcedVelocityMultiplier);
            } else {
    ...
    public ILensControl getLensController(ItemStack stack) {
        if (stack != null && stack.getItem() instanceof ILensControl) {
            ...
            if (control.isControlLens(stack)) return control;
        }
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        String name = StatCollector.translateToLocal(new ItemStack(ModBlocks.spreader, 1, getBlockMetadata())
                        .getUnlocalizedName()
    ...
    public boolean bindTo(EntityPlayer player, ItemStack wand, int x, int y, int z, int side) {
        ...
        AxisAlignedBB axis =
                player.worldObj.getBlock(x, y, z).getCollisionBoundingBoxFromPool(player.worldObj, x, y, z);
        if (axis == null) axis = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1);

        if (!blockVec.isInside(axis))
            blockVec = new Vector3(
    ...
    public UUID getIdentifier() {
        if (identity == null) identity = UUID.randomUUID();
        return identity;
    }
