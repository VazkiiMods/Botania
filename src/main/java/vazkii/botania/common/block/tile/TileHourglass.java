...
public class TileHourglass extends TileSimpleInventory {
        ...
        if (totalTime > 0) {
            ...
            if (time >= totalTime) {
                ...
                worldObj.scheduleBlockUpdate(
                        xCoord, yCoord, zCoord, getBlockType(), getBlockType().tickRate(worldObj));
            }
    ...
    public static int getStackItemTime(ItemStack stack) {
        if (stack == null) return 0;
        if (stack.getItem() == Item.getItemFromBlock(Blocks.sand)) return stack.getItemDamage() == 1 ? 200 : 20;
        if (stack.getItem() == Item.getItemFromBlock(Blocks.soul_sand)) return 1200;
        return 0;
    }
    ...
    public int getColor() {
        ...
        if (stack.getItem() == Item.getItemFromBlock(Blocks.soul_sand)) return 0x5A412f;
        return 0;
    }
    ...
    public void renderHUD(ScaledResolution res) {
        ...
        if (stack != null) {
            ...
            if (!move) status = status.isEmpty() ? "stopped" : "lockedStopped";
            if (!status.isEmpty())
                mc.fontRenderer.drawStringWithShadow(
                        StatCollector.translateToLocal("botaniamisc." + status), x + 20, y + 12, getColor());
        }
    }
