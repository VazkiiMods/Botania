...
public class TileRedStringContainer extends TileRedString implements ISidedInventory {
    ...
    public boolean acceptBlock(int x, int y, int z) {
        ...
        if (tile != null && tile instanceof IInventory) {
            ...
            if (inv instanceof ISidedInventory) {
                ...
                for (int i = 0; i < 6; i++) if (sidedInv.getAccessibleSlotsFromSide(i).length != 0) return true;
                return false;
            }
    ...
    public void setInventorySlotContents(int slot, ItemStack stack) {
        IInventory inv = getInventory();
        if (inv != null) inv.setInventorySlotContents(slot, stack);
    }
    ...
    public int[] getAccessibleSlotsFromSide(int side) {
        ...
        return inv instanceof ISidedInventory
                ? ((ISidedInventory) inv).getAccessibleSlotsFromSide(side)
                : inv instanceof IInventory ? InventoryHelper.buildSlotsForLinearInventory(inv) : new int[0];
    }
