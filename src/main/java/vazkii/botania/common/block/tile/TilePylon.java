...
public class TilePylon extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (activated && worldObj.isRemote) {
            if (worldObj.getBlock(centerX, centerY, centerZ) != getBlockForMeta()
                    || meta != 0 && worldObj.getBlockMetadata(centerX, centerY, centerZ) == 0) {
                activated = false;
                return;
            }

            Vector3 centerBlock =
                    new Vector3(centerX + 0.5, centerY + 0.75 + (Math.random() - 0.5 * 0.25), centerZ + 0.5);
            ...
            if (meta == 1) {
                if (ConfigHandler.elfPortalParticlesEnabled) {
                    ...
                    Vector3 movementVector =
                            centerBlock.sub(ourCoords).normalize().multiply(0.2);
                ...
                if (block == ModBlocks.flower || block == ModBlocks.shinyFlower) {
                    ...
                    if (worldObj.rand.nextInt(4) == 0)
                        Botania.proxy.sparkleFX(
                                worldObj,
                                centerBlock.x + (Math.random() - 0.5) * 0.5,
                                centerBlock.y,
                                centerBlock.z + (Math.random() - 0.5) * 0.5,
                                color[0],
                                color[1],
                                color[2],
                                (float) Math.random(),
                    ...
                    Botania.proxy.wispFX(
                            worldObj,
                            xCoord + 0.5 + (Math.random() - 0.5) * 0.25,
                            yCoord - 0.5,
                            zCoord + 0.5 + (Math.random() - 0.5) * 0.25,
                            color[0],
                            color[1],
                            color[2],
                    ...
                    Botania.proxy.wispFX(
                            worldObj,
                            xCoord + 0.5 + (Math.random() - 0.5) * 0.125,
                            yCoord + 1.5,
                            zCoord + 0.5 + (Math.random() - 0.5) * 0.125,
                            color[0],
                            color[1],
                            color[2],
