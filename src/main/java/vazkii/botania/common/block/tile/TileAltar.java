...
public class TileAltar extends TileSimpleInventory implements ISidedInventory, IPetalApothecary {
    ...
    private static final Pattern SEED_PATTERN =
            Pattern.compile("(?:(?:(?:[A-Z-_.:]|^)seed)|(?:(?:[a-z-_.:]|^)Seed))(?:[sA-Z-_.:]|$)");
    ...
    public boolean collideEntityItem(EntityItem item) {
        ...
        if (canGoInAltar(stack, this)) {
            ...
            if (!worldObj.isRemote) {
                ...
                && SEED_PATTERN
                        .matcher(stack.getItem().getUnlocalizedName(stack))
                        .find()) {
            for (RecipePetals recipe : BotaniaAPI.petalRecipes) {
                if (recipe.matches(this)) {
                    ...
                    if (!worldObj.isRemote) {
                        ...
                        EntityItem outputItem =
                                new EntityItem(worldObj, xCoord + 0.5, yCoord + 1.5, zCoord + 0.5, output);
    ...
    public static void tryToSetLastRecipe(EntityPlayer player, IInventory inv, List<ItemStack> lastRecipe) {
        if (lastRecipe == null || lastRecipe.isEmpty() || player.worldObj.isRemote) return;
        ...
        for (ItemStack stack : lastRecipe) {
            ...
            for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
                ...
                if (pstack != null && pstack.isItemEqual(stack) && ItemStack.areItemStackTagsEqual(stack, pstack)) {
                    pstack.stackSize--;
                    if (pstack.stackSize == 0) player.inventory.setInventorySlotContents(i, null);
    ...
    public void updateEntity() {
        List<EntityItem> items = worldObj.getEntitiesWithinAABB(
                ...
                AxisAlignedBB.getBoundingBox(
                        xCoord, yCoord + 1D / 16D * 20D, zCoord, xCoord + 1, yCoord + 1D / 16D * 21D, zCoord + 1));

        boolean didChange = false;

        for (EntityItem item : items) didChange = collideEntityItem(item) || didChange;

        if (didChange) VanillaPacketDispatcher.dispatchTEToNearbyPlayers(worldObj, xCoord, yCoord, zCoord);
            ...
            if (Math.random() >= 0.97) {
                ...
                if (Math.random() >= 0.75F)
                    worldObj.playSoundEffect(
                            xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, "game.neutral.swim.splash", 0.1F, 10F);
                Botania.proxy.sparkleFX(
                        worldObj,
                        xCoord + 0.5 + Math.random() * 0.4 - 0.2,
                        yCoord + 1,
            ...
            for (RecipePetals recipe : BotaniaAPI.petalRecipes)
                if (recipe.matches(this)) {
                    ...
                    RenderItem.getInstance()
                            .renderItemIntoGUI(mc.fontRenderer, mc.renderEngine, stack, xc + radius + 32, yc - 8);
                    RenderItem.getInstance()
                            .renderItemIntoGUI(
                                    mc.fontRenderer,
                                    mc.renderEngine,
