...
public class TilePool extends TileMod
                ...
                if (outputting && mana.canReceiveManaFromPool(stack, this)
                    ...
                    if (outputting)
                        for (ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS) {
                            TileEntity tile =
                                    worldObj.getTileEntity(xCoord + dir.offsetX, yCoord, zCoord + dir.offsetZ);
                            if (tile != null
                                    && tile instanceof TileBellows
                                    && ((TileBellows) tile).getLinkedTile() == this) bellowCount++;
                        }
                    ...
                    if (outputting) {
                        ...
                        if (canAccept) {
                            ...
                            int manaVal =
                                    Math.min(transfRate, Math.min(manaCap - getCurrentMana(), mana.getMana(stack)));
                            if (!worldObj.isRemote) mana.addMana(stack, -manaVal);
                            recieveMana(manaVal);
                        }
                    }
                    ...
                    if (didSomething) {
                        if (worldObj.isRemote
                                && ConfigHandler.chargingAnimationEnabled
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        ...
        String name = StatCollector.translateToLocal(
                pool.getUnlocalizedName().replaceAll("tile.", "tile." + LibResources.PREFIX_MOD) + ".name");
        int color = 0x4444FF;
