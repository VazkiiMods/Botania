...
public class TileAlfPortal extends TileMod {
    ...
    private static final int[][] GLIMMERING_LIVINGWOOD_POSITIONS = {{-2, 2, 0}, {2, 2, 0}, {0, 4, 0}};
    ...
    public static MultiblockSet makeMultiblockSet() {
        ...
        for (int[] l : LIVINGWOOD_POSITIONS) mb.addComponent(l[0], l[1] + 1, l[2], ModBlocks.livingwood, 0);
        for (int[] g : GLIMMERING_LIVINGWOOD_POSITIONS) mb.addComponent(g[0], g[1] + 1, g[2], ModBlocks.livingwood, 5);
        for (int[] p : PYLON_POSITIONS) mb.addComponent(-p[0], p[1] + 1, -p[2], ModBlocks.pylon, 1);
        for (int[] p : POOL_POSITIONS) mb.addComponent(-p[0], p[1] + 1, -p[2], ModBlocks.pool, 0);
            ...
            if (ticksOpen > 60) {
                ...
                if (!worldObj.isRemote)
                    for (EntityItem item : items) {
                        ...
                        if (stack != null
                                && (!(stack.getItem() instanceof IElvenItem)
                                        || !((IElvenItem) stack.getItem()).isElvenItem(stack))
                                && !item.getEntityData().hasKey(TAG_PORTAL_FLAG)) {
                            item.setDead();
                            addItem(stack);
                ...
                if (ticksSinceLastItem >= 4) {
                    if (!worldObj.isRemote) resolveRecipes();
                }
            }
        } else closeNow = false;
    ...
    private void blockParticle(int meta) {
        ...
        double[] pos =
                new double[] {AIR_POSITIONS[i][0] + 0.5F, AIR_POSITIONS[i][1] + 0.5F, AIR_POSITIONS[i][2] + 0.5F};
    ...
    AxisAlignedBB getPortalAABB() {
        AxisAlignedBB aabb =
                AxisAlignedBB.getBoundingBox(xCoord - 1, yCoord + 1, zCoord, xCoord + 2, yCoord + 4, zCoord + 1);
    ...
    void addItem(ItemStack stack) {
        ...
        stack.stackSize = 1;
        for (int i = 0; i < size; i++) stacksIn.add(stack.copy());
    }
    ...
    private boolean checkMultipleConverters(Function<int[], int[]>... converters) {
        ...
        if (!check2DArray(LIVINGWOOD_POSITIONS, ModBlocks.livingwood, 0, converters)) return false;
        if (!check2DArray(GLIMMERING_LIVINGWOOD_POSITIONS, ModBlocks.livingwood, 5, converters)) return false;
        if (!check2DArray(PYLON_POSITIONS, ModBlocks.pylon, 1, converters)) return false;
        if (!check2DArray(POOL_POSITIONS, ModBlocks.pool, -1, converters)) return false;
    ...
    private void lightPylons(Function<int[], int[]>... converters) {
        ...
        for (int[] pos : PYLON_POSITIONS) {
            ...
            if (tile instanceof TilePool) {
                ...
                if (pool.getCurrentMana() < cost) closeNow = true;
                else if (!worldObj.isRemote) pool.recieveMana(-cost);
            }
        }
    }
