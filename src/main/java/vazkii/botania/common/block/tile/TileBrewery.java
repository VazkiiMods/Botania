...
public class TileBrewery extends TileSimpleInventory implements ISidedInventory, IManaReceiver {
    ...
    public boolean addItem(EntityPlayer player, ItemStack stack) {
        if (recipe != null
                || stack == null
                || stack.getItem() instanceof IBrewItem
                        && ((IBrewItem) stack.getItem()).getBrew(stack) != null
                        && ((IBrewItem) stack.getItem()).getBrew(stack) != BotaniaAPI.fallbackBrew
                || getStackInSlot(0) == null != stack.getItem() instanceof IBrewContainer) return false;
                ...
                if (mana != manaLastTick) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        Botania.proxy.wispFX(
                                worldObj,
                                xCoord + 0.7 - Math.random() * 0.4,
                                yCoord + 0.9 - Math.random() * 0.2,
                                zCoord + 0.7 - Math.random() * 0.4,
                                r,
                                g,
                                b,
                                0.1F + (float) Math.random() * 0.05F,
                                0.03F - (float) Math.random() * 0.06F,
                                0.03F + (float) Math.random() * 0.015F,
                                0.03F - (float) Math.random() * 0.06F);
                        for (int j = 0; j < 2; j++)
                            Botania.proxy.wispFX(
                                    worldObj,
    ...
    public int getManaCost() {
        ...
        if (recipe == null || stack == null || !(stack.getItem() instanceof IBrewContainer)) return 0;
    ...
    public void craftingFanciness() {
        worldObj.playSoundEffect(
                xCoord, yCoord, zCoord, "botania:potionCreate", 1F, 1.5F + (float) Math.random() * 0.25F);
        for (int i = 0; i < 25; i++) {
            Color color = new Color(recipe.getBrew().getColor(getStackInSlot(0)));
            float r = color.getRed() / 255F;
            float g = color.getGreen() / 255F;
