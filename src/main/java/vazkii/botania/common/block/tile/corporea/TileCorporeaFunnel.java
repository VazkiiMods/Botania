...
public class TileCorporeaFunnel extends TileCorporeaBase implements ICorporeaRequestor {
    ...
    public void doRequest() {
        ...
        if (spark != null && spark.getMaster() != null) {
            ...
            if (!filter.isEmpty()) {
                ...
                if (stack != null) doCorporeaRequest(stack, stack.stackSize, spark);
            }
        }
    }
    ...
    public List<ItemStack> getFilter() {
        ...
        for (ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS) {
            List<EntityItemFrame> frames = worldObj.getEntitiesWithinAABB(
                    EntityItemFrame.class,
                    AxisAlignedBB.getBoundingBox(
                            xCoord + dir.offsetX,
                            yCoord + dir.offsetY,
                            zCoord + dir.offsetZ,
                            xCoord + dir.offsetX + 1,
                            yCoord + dir.offsetY + 1,
                            zCoord + dir.offsetZ + 1));
    ...
    public void doCorporeaRequest(Object request, int count, ICorporeaSpark spark) {
        if (!(request instanceof ItemStack)) return;
        ...
        for (ItemStack reqStack : stacks)
            if (request != null) {
                if (inv != null
                        && !(inv instanceof TileCorporeaFunnel)
                        && reqStack.stackSize
                                == InventoryHelper.testInventoryInsertion(inv, reqStack, ForgeDirection.UP))
