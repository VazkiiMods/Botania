...
public class TileEnchanter extends TileMod implements ISparkAttachable {
    ...
    public static MultiblockSet makeMultiblockSet() {
        ...
        for (int[] o : OBSIDIAN_LOCATIONS) mb.addComponent(o[0], o[1] + 1, o[2], Blocks.obsidian, 0);
    ...
    public void onWanded(EntityPlayer player, ItemStack wand) {
        ...
        List<EntityItem> items = worldObj.getEntitiesWithinAABB(
                EntityItem.class,
                AxisAlignedBB.getBoundingBox(xCoord - 2, yCoord, zCoord - 2, xCoord + 3, yCoord + 1, zCoord + 3));
    ...
    public void updateEntity() {
        if (getBlockMetadata() < PYLON_LOCATIONS.length)
            for (int[] pylon : PYLON_LOCATIONS[getBlockMetadata()]) {
                ...
                if (tile != null && tile instanceof TilePylon) ((TilePylon) tile).activated = false;
            }
        ...
        switch (stage) {
                ...
                if (stageTicks % 20 == 0) {
                    List<EntityItem> items = worldObj.getEntitiesWithinAABB(
                            ...
                            AxisAlignedBB.getBoundingBox(
                                    xCoord - 2, yCoord, zCoord - 2, xCoord + 3, yCoord + 1, zCoord + 3));
                    int count = items.size();
            ...
            case 2: { // Get Mana
                for (int[] pylon : PYLON_LOCATIONS[getBlockMetadata()]) {
                    TilePylon pylonTile =
                            (TilePylon) worldObj.getTileEntity(xCoord + pylon[0], yCoord + pylon[1], zCoord + pylon[2]);
                    if (pylonTile != null) {
                ...
                if (manaRequired == -1) {
                    ...
                    for (EnchantmentData data : enchants) {
                        ...
                        manaRequired += (int) (5000F
                                ...
                                * ((3F + data.level * data.level) * 0.25F)
                    ...
                    if (spark != null) {
                        List<ISparkEntity> sparkEntities =
                                SparkHelper.getSparksAround(worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5);
    ...
    public void writeCustomNBT(NBTTagCompound cmp) {
        ...
        String enchStr = "";
        for (EnchantmentData data : enchants) enchStr = enchStr + data.enchant + ":" + data.level + ",";
    ...
    private boolean hasEnchantAlready(int enchant) {
        for (EnchantmentData data : enchants) if (data.enchant == enchant) return true;
    ...
    public static boolean canEnchanterExist(World world, int x, int y, int z, int meta) {
        ...
        for (int[] pylon : PYLON_LOCATIONS[meta])
            if (world.getBlock(pylon[0] + x, pylon[1] + y, pylon[2] + z) != ModBlocks.pylon
                    || !BotaniaAPI.internalHandler.isBotaniaFlower(world, pylon[0] + x, pylon[1] + y - 1, pylon[2] + z))
                return false;
