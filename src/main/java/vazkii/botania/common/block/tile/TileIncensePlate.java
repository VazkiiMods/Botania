		...
		if(!stack.isEmpty() && burning) {
			...
			if(timeLeft > 0) {
				...
				if(!world.isRemote) {
                    ...
                    ParticleData data1 = ParticleData.wisp(0.05F + (float) Math.random() * 0.02F, r, g, b);
                    ...
                    ParticleData data = ParticleData.wisp(0.05F + (float) Math.random() * 0.02F, 0.2F, 0.2F, 0.2F);
