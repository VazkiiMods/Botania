...
public class BlockAltar extends BlockModContainer implements ILexiconable {
    ...
    public void getSubBlocks(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < 9; i++) list.add(new ItemStack(item, 1, i));
    }
    ...
    public boolean onBlockActivated(
        ...
        } else if (tile.isEmpty() && tile.hasWater && stack == null) tile.trySetLastRecipe(par5EntityPlayer);
        else {
            if (stack != null
    ...
    public void fillWithRain(World world, int x, int y, int z) {
        if (world.rand.nextInt(20) == 1) {
            ...
            if (tile instanceof TileAltar) {
                ...
                if (!altar.hasLava && !altar.hasWater) altar.setWater(true);
    ...
    private boolean isValidWaterContainer(ItemStack stack) {
        ...
        if (stack.getItem() == ModItems.waterBowl) return true;
