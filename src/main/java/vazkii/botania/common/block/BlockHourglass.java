...
public class BlockHourglass extends BlockModContainer implements IManaTrigger, IWandable, IWandHUD, ILexiconable {
    ...
    public boolean onBlockActivated(
            World world, int x, int y, int z, EntityPlayer player, int side, float xs, float ys, float zs) {
        ...
        if (hgStack == null && TileHourglass.getStackItemTime(stack) > 0) {
            ...
            if (!player.inventory.addItemStackToInventory(copy)) player.dropPlayerItemWithRandomChoice(copy, false);
