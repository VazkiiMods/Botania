...
public class BlockGhostRail extends BlockRailBase implements ILexiconable {
    ...
    public void onMinecartUpdate(MinecartUpdateEvent event) {
        ...
        if (block == this) event.entity.getEntityData().setInteger(TAG_FLOAT_TICKS, 20);
        else if (block instanceof BlockRailBase || block == ModBlocks.dreamwood) {
            event.entity.getEntityData().setInteger(TAG_FLOAT_TICKS, 0);
            if (floatTicks > 0) event.entity.worldObj.playAuxSFX(2003, x, y, z, 0);
        }
        ...
        if (floatTicks > 0) {
            ...
            if (air && airBelow || !air && !airBelow) event.entity.noClip = true;
