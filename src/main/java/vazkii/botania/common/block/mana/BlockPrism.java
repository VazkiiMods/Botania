...
public class BlockPrism extends BlockModContainer implements IManaTrigger, ILexiconable {
    ...
    public boolean onBlockActivated(
        ...
        if (!(tile instanceof TilePrism)) return false;
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        if (!world.isRemote) {
            if (power && !powered) world.setBlockMetadataWithNotify(x, y, z, meta | 8, 1 | 2);
            else if (!power && powered) world.setBlockMetadataWithNotify(x, y, z, meta & -9, 1 | 2);
        }
    }
    ...
    public void onBurstCollision(IManaBurst burst, World world, int x, int y, int z) {
        ...
        if (tile != null && tile instanceof TilePrism) ((TilePrism) tile).onBurstCollision(burst);
    }
