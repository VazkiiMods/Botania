...
public abstract class BlockCamo extends BlockModContainer<TileCamo> {
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        ...
        if (tile instanceof TileCamo) {
            ...
            if (block != null && isValidBlock(block)) return block.getIcon(side, camo.camoMeta);
        }
    ...
    public boolean onBlockActivated(
        ...
        if (tile instanceof TileCamo) {
            ...
            if (currentStack == null) currentStack = new ItemStack(Block.getBlockFromName("air"), 1, 0);
    ...
    public int colorMultiplier(IBlockAccess par1World, int par2, int par3, int par4) {
        ...
        if (tile instanceof TileCamo) {
            TileCamo camo = (TileCamo) tile;
            Block block = camo.camo;
            if (block != null) return block instanceof BlockCamo ? 0xFFFFFF : block.getRenderColor(camo.camoMeta);
        }
        return 0xFFFFFF;
    }
