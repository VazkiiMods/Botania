...
public class BlockFakeAir extends BlockModContainer {
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        if (shouldRemove(world, x, y, z)) world.scheduleBlockUpdate(x, y, z, this, tickRate(world));
    }

    private boolean shouldRemove(World world, int x, int y, int z) {
        return !world.isRemote && world.getTileEntity(x, y, z) == null
                || !(world.getTileEntity(x, y, z) instanceof TileFakeAir)
                || !((TileFakeAir) world.getTileEntity(x, y, z)).canStay();
    }

    @Override
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (shouldRemove(world, x, y, z)) world.setBlock(x, y, z, Blocks.water);
    }
