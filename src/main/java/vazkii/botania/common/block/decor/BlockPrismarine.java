...
public class BlockPrismarine extends BlockMod implements ILexiconable {
    ...
    public void getSubBlocks(Item par1, CreativeTabs par2CreativeTabs, List par3List) {
        for (int i = 0; i < TYPES; i++) par3List.add(new ItemStack(par1, 1, i));
    }
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 1; i < TYPES; i++) icons[i] = IconHelper.forBlock(par1IconRegister, this, i);
    }
    ...
    public void loadTextures(TextureStitchEvent.Pre event) {
        if (event.map.getTextureType() == 0) {
            ...
            if (event.map.setTextureEntry("botania:prismarine0", icon)) icons[0] = icon;
        }
    }
