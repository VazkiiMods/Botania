...
public class BlockSpecialQuartz extends BlockMod implements ILexiconable {
    ...
    public BlockSpecialQuartz(String type) {
        ...
        iconNames = new String[] {
            "block" + type + "Quartz0", "chiseled" + type + "Quartz0", "pillar" + type + "Quartz0", null, null
        };
        setHardness(0.8F);
        setResistance(10F);
    ...
    public IIcon getIcon(int par1, int par2) {
        if (par2 != 2 && par2 != 3 && par2 != 4) {
            if (par1 != 1 && (par1 != 0 || par2 != 1)) {
                if (par1 == 0) return specialQuartzTopIcon;
                else {
                    if (par2 < 0 || par2 >= specialQuartzIcons.length) par2 = 0;
            ...
            return par2 == 2 && (par1 == 1 || par1 == 0)
                    ? pillarSpecialQuartzIcon
                    : par2 == 3 && (par1 == 5 || par1 == 4)
                            ? pillarSpecialQuartzIcon
                            : par2 == 4 && (par1 == 2 || par1 == 3)
                                    ? pillarSpecialQuartzIcon
                                    : specialQuartzIcons[par2];
    }
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < specialQuartzIcons.length; ++i) {
            if (iconNames[i] == null) specialQuartzIcons[i] = specialQuartzIcons[i - 1];
