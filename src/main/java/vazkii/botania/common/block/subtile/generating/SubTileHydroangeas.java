...
public class SubTileHydroangeas extends SubTilePassiveGenerating {
        ...
        if (cooldown > 0) {
            ...
            for (int i = 0; i < 3; i++)
                Botania.proxy.wispFX(
                        ...
                        supertile.xCoord + 0.5 + Math.random() * 0.2 - 0.1,
                        supertile.yCoord + 0.5 + Math.random() * 0.2 - 0.1,
                        supertile.zCoord + 0.5 + Math.random() * 0.2 - 0.1,
                        0.1F,
                        0.1F,
                        0.1F,
                        (float) Math.random() / 6,
        ...
        if (burnTime == 0) {
            if (mana < getMaxMana() && !supertile.getWorldObj().isRemote) {
                ...
                for (int[] offsetArray : offsets) {
                    int[] positions = {supertile.xCoord + offsetArray[0], supertile.zCoord + offsetArray[1]};

                    Material search = getMaterialToSearchFor();
                    if (supertile
                                            .getWorldObj()
                                            .getBlock(positions[0], supertile.yCoord, positions[1])
                                            .getMaterial()
                                    == search
                            && (getBlockToSearchBelow() == null
                                    || supertile
                                                    .getWorldObj()
                                                    .getBlock(positions[0], supertile.yCoord - 1, positions[1])
                                            == getBlockToSearchBelow())
                            && supertile.getWorldObj().getBlockMetadata(positions[0], supertile.yCoord, positions[1])
                                    == 0) {
                        if (search != Material.water)
                            ...
                            for (ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS)
                                if (supertile
                                                .getWorldObj()
                                                .getBlock(
                                                        positions[0] + dir.offsetX,
                                                        supertile.yCoord,
                                                        positions[1] + dir.offsetZ)
                                                .getMaterial()
                                        == search) waterAround++;
                        ...
                        if (cooldown == 0) burnTime += getBurnTime();
    ...
    public void doBurnParticles() {
        Botania.proxy.wispFX(
                ...
                supertile.yCoord + 0.55 + Math.random() * 0.2 - 0.1,
                supertile.zCoord + 0.5,
                0.05F,
                0.05F,
                0.7F,
                (float) Math.random() / 6,
    ...
    public void populateDropStackNBTs(List<ItemStack> drops) {
        ...
        if (burnTime > 0) cooldown = getCooldown();

        if (cooldown > 0) ItemNBTHelper.setInt(drops.get(0), TAG_COOLDOWN, getCooldown());
    }
    ...
    public int getDelayBetweenPassiveGeneration() {
        boolean rain = supertile
                                .getWorldObj()
                                .getWorldChunkManager()
                                .getBiomeGenAt(supertile.xCoord, supertile.zCoord)
                                .getIntRainfall()
                        > 0
                && (supertile.getWorldObj().isRaining()
                        || supertile.getWorldObj().isThundering());
        return rain ? 1 : 2;
    }
