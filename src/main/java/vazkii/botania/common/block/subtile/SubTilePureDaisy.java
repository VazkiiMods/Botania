...
public class SubTilePureDaisy extends SubTileEntity {
    ...
    public void onUpdate() {
        ...
        ChunkCoordinates coords = new ChunkCoordinates(
                supertile.xCoord + acoords[0], supertile.yCoord + acoords[1], supertile.zCoord + acoords[2]);
        ...
        if (!world.isAirBlock(coords.posX, coords.posY, coords.posZ)) {
            ...
            if (recipe != null) {
                ...
                Botania.proxy.sparkleFX(
                        ...
                        coords.posX + Math.random(),
                        coords.posY + Math.random(),
                        coords.posZ + Math.random(),
                        1F,
                        1F,
                        1F,
                        (float) Math.random(),
                    ...
                    if (recipe.set(world, coords.posX, coords.posY, coords.posZ, this)) {
                        for (int i = 0; i < 25; i++) {
                            ...
                            Botania.proxy.wispFX(
                                    supertile.getWorldObj(), x, y, z, 1F, 1F, 1F, (float) Math.random() / 2F);
                        }
                        if (ConfigHandler.blockBreakParticles)
                            supertile
                                    ...
                                    .playAuxSFX(
                                            ...
                                            coords.posY,
                                            coords.posZ,
                                            Block.getIdFromBlock(recipe.getOutput()) + (recipe.getOutputMeta() << 12));
                    }
                }
    ...
    public void readFromPacketNBT(NBTTagCompound cmp) {
        ...
        if (supertile.getWorldObj() != null && !supertile.getWorldObj().isRemote)
            for (int i = 0; i < ticksRemaining.length; i++) ticksRemaining[i] = cmp.getInteger(TAG_TICKS_REMAINING + i);
    }

    @Override
    public void writeToPacketNBT(NBTTagCompound cmp) {
        cmp.setInteger(TAG_POSITION, positionAt);
        for (int i = 0; i < ticksRemaining.length; i++) cmp.setInteger(TAG_TICKS_REMAINING + i, ticksRemaining[i]);
    }
