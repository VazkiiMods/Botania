...
public class SubTileMunchdew extends SubTileGenerating {
    ...
    public void onUpdate() {
        ...
        eatLeaves:
        {
            if (getMaxMana() - mana >= manaPerLeaf && !supertile.getWorldObj().isRemote && ticksExisted % 4 == 0) {
                        ...
                        for (int k = -RANGE; k < RANGE + 1; k++) {
                            ...
                            if (block.getMaterial() == Material.leaves) {
                                ...
                                for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS)
                                    if (supertile
                                            .getWorldObj()
                                            .getBlock(xp + dir.offsetX, yp + dir.offsetY, zp + dir.offsetZ)
                                            .isAir(
                                ...
                                if (exposed) coords.add(new ChunkCoordinates(xp, yp, zp));
                            }
                        }

                if (coords.isEmpty()) break eatLeaves;
                ...
                int meta =
                        supertile.getWorldObj().getBlockMetadata(breakCoords.posX, breakCoords.posY, breakCoords.posZ);
        ...
        if (ateOnce) {
            ticksWithoutEating++;
            if (ticksWithoutEating >= 5) cooldown = 1600;
        }
    }
    ...
    public ArrayList<ItemStack> getDrops(ArrayList<ItemStack> list) {
        ...
        if (cooldown > 0) ItemNBTHelper.setInt(drops.get(0), TAG_COOLDOWN, cooldown);
        return drops;
    }
