...
public class SubTileBubbell extends SubTileFunctional {
    ...
    public void onUpdate() {
        ...
        if (mana > COST_PER_TICK) {
            ...
            if (ticksExisted % 10 == 0 && range < getRange()) range++;

            for (int i = -range; i < range + 1; i++)
                for (int j = -range; j < range + 1; j++)
                    for (int k = -range; k < range + 1; k++)
                        if (MathHelper.pointDistanceSpace(i, j, k, 0, 0, 0) < range) {
                            Block block = supertile
                                    .getWorldObj()
                                    .getBlock(supertile.xCoord + i, supertile.yCoord + j, supertile.zCoord + k);
                            if (block.getMaterial() == Material.water) {
                                supertile
                                        ...
                                        .setBlock(
                                                ...
                                                0,
                                                2);
                                TileFakeAir air = (TileFakeAir) supertile
                                        .getWorldObj()
                                        .getTileEntity(
                                                supertile.xCoord + i, supertile.yCoord + j, supertile.zCoord + k);
