...
public class SubTileSolegnolia extends SubTileFunctional {
    ...
    public static boolean hasSolegnoliaAround(Entity e) {
        for (SubTileSolegnolia flower : existingFlowers) {
            if (flower.redstoneSignal > 0
                    || flower.supertile.getWorldObj() != e.worldObj
                    || flower.supertile
                                    .getWorldObj()
                                    .getTileEntity(
                                            flower.supertile.xCoord, flower.supertile.yCoord, flower.supertile.zCoord)
                            != flower.supertile) continue;

            double range = flower.getRange();
            if (MathHelper.pointDistanceSpace(
                            e.posX,
                            e.posY,
                            e.posZ,
                            flower.supertile.xCoord + 0.5,
                            flower.supertile.yCoord + 0.5,
                            flower.supertile.zCoord + 0.5)
                    <= range) return true;
        }
