...
public class SubTileMarimorphosis extends SubTileFunctional {
    ...
    private static final Type[] TYPES = new Type[] {
        Type.FOREST, Type.PLAINS, Type.MOUNTAIN, Type.MUSHROOM, Type.SWAMP, Type.SANDY, Type.COLD, Type.MESA
    };
    ...
    public ItemStack getStoneToPut(ChunkCoordinates coords) {
        List<Type> types = Arrays.asList(BiomeDictionary.getTypesForBiome(
                supertile.getWorldObj().getBiomeGenForCoords(coords.posX, coords.posZ)));

        List<Integer> values = new ArrayList();
        for (int i = 0; i < 8; i++) {
            int times = 1;
            if (types.contains(TYPES[i])) times = 12;

            for (int j = 0; j < times; j++) values.add(i);
        }

        return new ItemStack(
                ModFluffBlocks.biomeStoneA,
                1,
                values.get(supertile.getWorldObj().rand.nextInt(values.size())));
    }
    ...
    public ChunkCoordinates getCoordsToPut() {
        ...
        if (possibleCoords.isEmpty()) return null;
