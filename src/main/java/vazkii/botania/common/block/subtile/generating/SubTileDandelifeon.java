...
public class SubTileDandelifeon extends SubTileGenerating {
    ...
    public void onUpdate() {
        ...
        if (!supertile.getWorldObj().isRemote && redstoneSignal > 0 && ticksExisted % SPEED == 0) runSimulation();
    }
    ...
    void runSimulation() {
        ...
        for (int i = 0; i < table.length; i++)
            for (int j = 0; j < table[0].length; j++) {
                ...
                int newVal = gen;
                if (adj < 2 || adj > 3) newVal = -1;
                else {
                    if (adj == 3 && gen == -1) newVal = getSpawnCellGeneration(table, i, j);
                    else if (gen > -1) newVal = gen + 1;
                }
    ...
    void setBlockForGeneration(int x, int y, int z, int gen, int prevGen) {
        ...
        if (gen == -2) {
            ...
            if (gen < 0 || gen > MAX_GENERATIONS) world.setBlockToAir(x, y, z);
