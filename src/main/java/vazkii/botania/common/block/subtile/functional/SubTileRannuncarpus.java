...
public class SubTileRannuncarpus extends SubTileFunctional {
            ...
            List<EntityItem> items = supertile
                    ...
                    .getEntitiesWithinAABB(
                            ...
                            AxisAlignedBB.getBoundingBox(
                                    x - RANGE, y - RANGE_Y, z - RANGE, x + RANGE + 1, y + RANGE_Y, z + RANGE + 1));
            int slowdown = getSlowdownFactor();
            for (EntityItem item : items) {
                if (item.age < (60 + slowdown) || item.isDead) continue;

                ItemStack stack = item.getEntityItem();
                Item stackItem = stack.getItem();
                if (stackItem instanceof ItemBlock
                        || stackItem instanceof ItemReed
                        ...
                        for (int i = -rangePlace; i < rangePlace + 1; i++)
                            for (int j = -rangePlaceY; j < rangePlaceY + 1; j++)
                                for (int l = -rangePlace; l < rangePlace + 1; l++) {
                                    ...
                                    if (filter.equals(supertile.getWorldObj(), xp, yp, zp)
                                            && (blockAbove.isAir(supertile.getWorldObj(), xp, yp + 1, zp)
                                                    || blockAbove.isReplaceable(
                                                            supertile.getWorldObj(), xp, yp + 1, zp)))
                    ...
                    if (!validPositions.isEmpty() && !supertile.getWorldObj().isRemote) {
                        ChunkCoordinates coords =
                                validPositions.get(supertile.getWorldObj().rand.nextInt(validPositions.size()));

                        Block blockToPlace = null;
                        if (stackItem instanceof IFlowerPlaceable)
                            blockToPlace = ((IFlowerPlaceable) stackItem)
                                    .getBlockToPlaceByFlower(stack, this, coords.posX, coords.posY, coords.posZ);
                        if (stackItem instanceof ItemBlock) blockToPlace = ((ItemBlock) stackItem).field_150939_a;
                        else if (stackItem instanceof ItemReed)
                            blockToPlace = ReflectionHelper.getPrivateValue(
                                    ItemReed.class, (ItemReed) stackItem, LibObfuscation.REED_ITEM);
                        else if (stackItem instanceof ItemRedstone) blockToPlace = Blocks.redstone_wire;

                        if (blockToPlace != null) {
                            if (blockToPlace.canPlaceBlockAt(
                                    supertile.getWorldObj(), coords.posX, coords.posY, coords.posZ)) {
                                supertile
                                        .getWorldObj()
                                        .setBlock(
                                                coords.posX,
                                ...
                                if (ConfigHandler.blockBreakParticles)
                                    supertile
                                            ...
                                            .playAuxSFX(
                                                    ...
                                                    coords.posY,
                                                    coords.posZ,
                                                    Block.getIdFromBlock(blockToPlace) + (stack.getItemDamage() << 12));
                                validPositions.remove(coords);

                                TileEntity tile =
                                        supertile.getWorldObj().getTileEntity(coords.posX, coords.posY, coords.posZ);
                                ...
                                if (stackItem instanceof IFlowerPlaceable)
                                    ((IFlowerPlaceable) stackItem)
                                            .onBlockPlacedByFlower(stack, this, coords.posX, coords.posY, coords.posZ);
    ...
    public BlockData getUnderlyingBlock() {
        return new BlockData(
                supertile.getWorldObj(),
                supertile.xCoord,
                supertile.yCoord - (supertile instanceof IFloatingFlower ? 1 : 2),
                supertile.zCoord);
    }
    ...
    public static class Mini extends SubTileRannuncarpus {
        @Override
        public int getRange() {
            return mana > 0 ? RANGE_PLACE_MANA_MINI : RANGE_PLACE_MINI;
        }
