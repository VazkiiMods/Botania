    ...
    public void onUpdate() {
        ...
        if (mana >= COST && !supertile.getWorldObj().isRemote && ticksExisted % 30 == 0) {
            ...
            for (int i = 0; i < RANGE * 2; i++) {
                ...
                if ((supertile.getWorldObj().isAirBlock(x, y + 1, z)
                                || blockAbove.isReplaceable(supertile.getWorldObj(), x, y + 1, z))
                        && blockAbove.getMaterial() != Material.water
                        && ModBlocks.flower.canPlaceBlockAt(supertile.getWorldObj(), x, y + 1, z)) {
                    ...
                    if (ModBlocks.flower.canBlockStay(supertile.getWorldObj(), x, y + 1, z)) {
                        if (ConfigHandler.blockBreakParticles)
                            supertile
                                    .getWorldObj()
                                    .playAuxSFX(
                                            2001, x, y + 1, z, Block.getIdFromBlock(ModBlocks.flower) + (color << 12));
