    ...
    public void onUpdate() {
        ...
        if (mana > 0) {
            ...
            AxisAlignedBB boundingBox = AxisAlignedBB.getBoundingBox(
                    x1 - range, y1 - range, z1 - range, x1 + range + 1, y1 + range + 1, z1 + range + 1);
            List<EntityLivingBase> entities =
                    supertile.getWorldObj().getEntitiesWithinAABB(EntityLivingBase.class, boundingBox);

            for (EntityLivingBase entity : entities) {
                if (entity instanceof EntityPlayer || entity instanceof IBossDisplayData) continue;
                ...
                if (distance > maxDist && distance < range) {
                    ...
                    if (supertile.getWorldObj().rand.nextInt(3) == 0)
                        Botania.proxy.sparkleFX(
                                ...
                                x2 + Math.random() * entity.width,
                                y2 + Math.random() * entity.height,
                                z2 + Math.random() * entity.width,
                                0.5F,
                                0.5F,
                                0.5F,
                                1F,
