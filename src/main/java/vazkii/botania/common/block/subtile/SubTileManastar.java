...
public class SubTileManastar extends SubTileEntity {
	...
	public void onUpdate() {
		...
		if(manaLastTick != -1 && mana != manaLastTick && Math.random() > 0.6) {
			...
			Botania.proxy.wispFX(supertile.getWorld(), supertile.xCoord + 0.55 + Math.random() * 0.2 - 0.1, supertile.yCoord + 0.75 + Math.random() * 0.2 - 0.1, supertile.zCoord + 0.5, more ? 0.05F : 1F, 0.05F, more ? 1F : 0.05F, (float) Math.random() / 7, (float) -Math.random() / 50);
		}
