...
public class SubTileArcaneRose extends TileEntityGeneratingFlower {
	...
	public void tickFlower() {
		...
		AxisAlignedBB effectBounds = new AxisAlignedBB(getEffectivePos().add(-RANGE, -RANGE, -RANGE), getEffectivePos().add(RANGE + 1, RANGE + 1, RANGE + 1));

		List<PlayerEntity> players = getWorld().getEntitiesWithinAABB(PlayerEntity.class, effectBounds);
		...
		List<ExperienceOrbEntity> orbs = getWorld().getEntitiesWithinAABB(ExperienceOrbEntity.class, effectBounds);
		for (ExperienceOrbEntity orb : orbs) {
			if (orb.isAlive()) {
				addMana(orb.getXpValue() * MANA_PER_XP_ORB);
				orb.remove();
