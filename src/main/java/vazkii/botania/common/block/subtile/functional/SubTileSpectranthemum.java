...
public class SubTileSpectranthemum extends SubTileFunctional {
            ...
            List<EntityItem> items = supertile
                    ...
                    .getEntitiesWithinAABB(
                            ...
                            AxisAlignedBB.getBoundingBox(
                                    x - RANGE, y - RANGE, z - RANGE, x + RANGE + 1, y + RANGE, z + RANGE + 1));
            int slowdown = getSlowdownFactor();

            for (EntityItem item : items) {
                if (item.age < (60 + slowdown)
                        || item.isDead
                        || item.getEntityData().getBoolean(TAG_TELEPORTED)) continue;

                ItemStack stack = item.getEntityItem();
                if (stack != null) {
                    Item sitem = stack.getItem();
                    if (sitem instanceof IManaItem) continue;
    ...
    public static void spawnExplosionParticles(EntityItem item, int p) {
        for (int i = 0; i < p; i++) {
            ...
            item.worldObj.spawnParticle(
                    "explode",
                    item.posX + item.worldObj.rand.nextFloat() * item.width * 2.0F - item.width - d0 * d3,
                    item.posY + item.worldObj.rand.nextFloat() * item.height - d1 * d3,
                    item.posZ + item.worldObj.rand.nextFloat() * item.width * 2.0F - item.width - d2 * d3,
                    d0,
                    d1,
                    d2);
        }
    ...
    public boolean bindTo(EntityPlayer player, ItemStack wand, int x, int y, int z, int side) {
        ...
        if (!bound
                && (x != bindX || y != bindY || z != bindZ)
                && MathHelper.pointDistanceSpace(x, y, z, supertile.xCoord, supertile.yCoord, supertile.zCoord)
                        <= BIND_RANGE
                && (x != supertile.xCoord || y != supertile.yCoord || z != supertile.zCoord)) {
            bindX = x;
            bindY = y;
            bindZ = z;
            sync();
    ...
    public ChunkCoordinates getBinding() {
        return Minecraft.getMinecraft().thePlayer.isSneaking() && bindY != -1
                ? new ChunkCoordinates(bindX, bindY, bindZ)
                : super.getBinding();
    }
