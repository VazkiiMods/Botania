...
public class ItemLokiRing extends ItemRelicBauble implements IExtendedWireframeCoordinateListProvider, IManaUsingItem {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (lokiRing == null || player.worldObj.isRemote) return;
        ...
        if (heldItemStack == null && event.action == Action.RIGHT_CLICK_BLOCK && player.isSneaking()) {
            if (originCoords.posY == -1 && lookPos != null) {
                ...
                if (originCoords.posX == lookPos.blockX
                        && originCoords.posY == lookPos.blockY
                        && originCoords.posZ == lookPos.blockZ) {
                            ...
                            if (cursor.posX == relX && cursor.posY == relY && cursor.posZ == relZ) {
                                ...
                                if (player instanceof EntityPlayerMP)
                                    PacketHandler.INSTANCE.sendTo(
                                            new PacketSyncBauble(player, slot), (EntityPlayerMP) player);
                                break addCursor;
                            }
            ...
            for (ChunkCoordinates cursor : cursors) {
                ...
                if (!player.worldObj.isAirBlock(x, y, z)
                    ...
                    item.onItemUse(
                            player.capabilities.isCreativeMode ? heldItemStack.copy() : heldItemStack,
                            player,
                            player.worldObj,
                            x,
                            y,
                            z,
                            lookPos.sideHit,
                            (float) lookPos.hitVec.xCoord - x,
                            (float) lookPos.hitVec.yCoord - y,
                            (float) lookPos.hitVec.zCoord - z);
    ...
    public static void breakOnAllCursors(
            EntityPlayer player, Item item, ItemStack stack, int x, int y, int z, int side) {
        ItemStack lokiRing = getLokiRing(player);
        if (lokiRing == null || player.worldObj.isRemote || !(item instanceof ISequentialBreaker)) return;
        ...
        for (int i = 0; i < cursors.size(); i++) {
            ...
            ToolCommons.removeBlockWithDrops(
                    player,
                    stack,
                    player.worldObj,
                    xp,
                    ...
                    z,
                    block,
                    new Material[] {block.getMaterial()},
                    silk,
                    fortune,
                    block.getBlockHardness(world, xp, yp, zp),
                    dispose);
        }
    }
    ...
    public List<ChunkCoordinates> getWireframesToDraw(EntityPlayer player, ItemStack stack) {
        if (getLokiRing(player) != stack) return null;

        MovingObjectPosition lookPos = Minecraft.getMinecraft().objectMouseOver;

        if (lookPos != null
                && !player.worldObj.isAirBlock(lookPos.blockX, lookPos.blockY, lookPos.blockZ)
                && lookPos.entityHit == null) {
