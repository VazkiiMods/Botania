...
public class ItemDice extends ItemRelic {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (isRightPlayer(player, stack) && !player.worldObj.isRemote) {
            ...
            if (hasRelicAlready(player, relic)) {
                ...
                for (int i = 0; i < 6; i++)
                    if (hasRelicAlready(player, i)) alreadyHas.add(i);
                    else possible.add(i);

                if (alreadyHas.size() > 0) possible.add(alreadyHas.get(world.rand.nextInt(alreadyHas.size())));
            ...
            if (hasRelicAlready(player, relic)) {
                player.addChatMessage(new ChatComponentTranslation("botaniamisc.dudDiceRoll", relic + 1)
                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_GREEN)));
                stack.stackSize--;
                return stack;
            }

            player.addChatMessage(new ChatComponentTranslation("botaniamisc.diceRoll", relic + 1)
                    .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_GREEN)));
    ...
    boolean hasRelicAlready(EntityPlayer player, int relic) {
        if (relic < 0 || relic > 5 || !(player instanceof EntityPlayerMP)) return true;
