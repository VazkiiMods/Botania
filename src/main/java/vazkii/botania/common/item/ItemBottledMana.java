...
public class ItemBottledMana extends ItemMod {
    ...
    public void effect(EntityPlayer player, int id) {
        switch (id) {
            ...
            case 2: { // Set on Fire
                if (!player.worldObj.isRemote) player.setFire(4);
                break;
            }
            ...
            case 5: { // Randomly set HP
                if (!player.worldObj.isRemote) player.setHealth(player.worldObj.rand.nextInt(19) + 1);
                break;
            }
                ...
                if (!player.worldObj.isRemote)
                    for (int i = 0; i < player.inventory.getSizeInventory(); i++)
                        if (i != player.inventory.currentItem) {
                            ...
                            if (stackAt != null) player.dropPlayerItemWithRandomChoice(stackAt, true);
                    ...
                    if (!block.isAir(player.worldObj, x, i, z)) {
                        if (player instanceof EntityPlayerMP) {
                            ...
                            mp.playerNetServerHandler.setPlayerLocation(
                                    player.posX, i + 1.6, player.posZ, player.rotationYaw, player.rotationPitch);
                        }
                        break;
                    }
                }
            ...
            case 10: { // HYPERSPEEEEEED
                if (!player.worldObj.isRemote) player.addPotionEffect(new PotionEffect(Potion.moveSpeed.id, 60, 200));
                break;
            }
            case 11: { // Night Vision
                if (!player.worldObj.isRemote) player.addPotionEffect(new PotionEffect(Potion.nightVision.id, 6000, 0));
                break;
            }
            case 12: { // Flare
                if (!player.worldObj.isRemote) {
                    ...
                    player.worldObj.playSoundAtEntity(
                            ...
                            "random.explode",
                            40F,
                            (1.0F + (player.worldObj.rand.nextFloat() - player.worldObj.rand.nextFloat()) * 0.2F)
                                    * 0.7F);
    ...
    long getSeed(ItemStack stack) {
        ...
        if (seed == -1) return randomSeed(stack);
        return seed;
    }
    ...
    public ItemStack onEaten(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        ...
        if (par1ItemStack.getItemDamage() == 6) return new ItemStack(Items.glass_bottle);
        return par1ItemStack;
    }
