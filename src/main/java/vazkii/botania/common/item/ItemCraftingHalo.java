...
public class ItemCraftingHalo extends ItemMod implements ICraftAchievement {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (segment == 0) player.openGui(Botania.instance, LibGuiIDs.CRAFTING_HALO, world, 0, 0, 0);
        else {
            if (itemForPos == null) assignRecipe(stack, itemForPos, segment);
    ...
    void tryCraft(EntityPlayer player, ItemStack stack, int slot, boolean particles, IInventory inv, boolean validate) {
        ...
        if (itemForPos == null) return;

        ItemStack[] recipe = getCraftingItems(stack, slot);
        if (validate) recipe = validateRecipe(player, stack, recipe, slot);

        if (canCraft(player, recipe, inv)) doCraft(player, recipe, particles);
    }

    private static ItemStack[] validateRecipe(EntityPlayer player, ItemStack stack, ItemStack[] recipe, int slot) {
        InventoryCrafting fakeInv =
                new InventoryCrafting(new ContainerWorkbench(player.inventory, player.worldObj, 0, 0, 0), 3, 3);
        for (int i = 0; i < 9; i++) fakeInv.setInventorySlotContents(i, recipe[i]);
        ...
        if (!result.isItemEqual(recipe[9])
                || result.stackSize != recipe[9].stackSize
                || !ItemStack.areItemStackTagsEqual(recipe[9], result)) {
            assignRecipe(stack, recipe[9], slot);
            return null;
        }
    ...
    private static boolean consumeRecipeIngredients(ItemStack[] recipe, IInventory inv, EntityPlayer player) {
        for (int i = 0; i < 9; i++) {
            ...
            if (ingredient != null && !consumeFromInventory(ingredient, inv, player)) return false;
        }
    ...
    private static boolean consumeFromInventory(ItemStack stack, IInventory inv, EntityPlayer player) {
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            ...
            if (stackAt != null && stack.isItemEqual(stackAt) && ItemStack.areItemStackTagsEqual(stack, stackAt)) {
                ...
                if (container != null) {
                    if (container == stackAt) consume = false;
                    else {
                ...
                if (consume) {
                    stackAt.stackSize--;
                    if (stackAt.stackSize == 0) inv.setInventorySlotContents(i, null);
                }
    ...
    public static ItemStack getItemForSlot(ItemStack stack, int slot) {
        if (slot == 0) return craftingTable;
        else if (slot >= SEGMENTS) return null;
        else {
    ...
    public static void assignRecipe(ItemStack stack, ItemStack itemForPos, int pos) {
        if (itemForPos != null) ItemNBTHelper.setCompound(stack, TAG_STORED_RECIPE_PREFIX + pos, new NBTTagCompound());
        else ItemNBTHelper.setCompound(stack, TAG_STORED_RECIPE_PREFIX + pos, getLastCraftingCompound(stack, false));
    }

    @SubscribeEvent
    public void onItemCrafted(ItemCraftedEvent event) {
        if (!(event.craftMatrix instanceof InventoryCraftingHalo)) return;

        for (int i = 0; i < event.player.inventory.getSizeInventory(); i++) {
            ItemStack stack = event.player.inventory.getStackInSlot(i);
            if (stack != null && stack.getItem() instanceof ItemCraftingHalo) saveRecipeToStack(event, stack);
        }
    }
    ...
    public static ItemStack[] getCraftingItems(ItemStack stack, int slot) {
        ...
        NBTTagCompound cmp = getStoredRecipeCompound(stack, slot);
        if (cmp != null) for (int i = 0; i < stackArray.length; i++) stackArray[i] = getLastCraftingItem(cmp, i);
    ...
    public static NBTTagCompound getStoredRecipeCompound(ItemStack stack, int slot) {
        return slot == SEGMENTS
                ? getLastCraftingCompound(stack, true)
                : ItemNBTHelper.getCompound(stack, TAG_STORED_RECIPE_PREFIX + slot, true);
    }
    ...
    public void onRenderWorldLast(RenderWorldLastEvent event) {
        ...
        if (stack != null && stack.getItem() instanceof ItemCraftingHalo) render(stack, player, event.partialTicks);
    }
    ...
    public void render(ItemStack stack, EntityPlayer player, float partialTicks) {
        ...
        float alpha =
                ((float) Math.sin((ClientTickHandler.ticksInGame + partialTicks) * 0.2F) * 0.5F + 0.5F) * 0.4F + 0.3F;
        ...
        for (int seg = 0; seg < SEGMENTS; seg++) {
            ...
            if (slotStack != null) {
                mc.renderEngine.bindTexture(
                        slotStack.getItem() instanceof ItemBlock
                                ? TextureMap.locationBlocksTexture
                                : TextureMap.locationItemsTexture);

                if (slotStack.getItem() instanceof ItemBlock
                        && RenderBlocks.renderItemIn3d(
                                Block.getBlockFromItem(slotStack.getItem()).getRenderType())) {
                    ...
                    RenderBlocks.getInstance()
                            .renderBlockAsItem(
                                    Block.getBlockFromItem(slotStack.getItem()), slotStack.getItemDamage(), 1F);
                } else {
            ...
            if (seg % 2 == 0) GL11.glColor4f(0.6F, 0.6F, 0.6F, a);
    ...
    public static void renderHUD(ScaledResolution resolution, EntityPlayer player, ItemStack stack) {
        ...
        if (slot == 0) {
            ...
            if (recipe[9] == null) recipe = getCraftingItems(stack, SEGMENTS);
            else {
    ...
    @SideOnly(Side.CLIENT)
    public static void renderRecipe(
            ScaledResolution resolution, String label, ItemStack[] recipe, EntityPlayer player, boolean setRecipe) {
        Minecraft mc = Minecraft.getMinecraft();
        ...
        if (recipe[9] != null) {
            ...
            RenderItem.getInstance()
                    .renderItemAndEffectIntoGUI(mc.fontRenderer, mc.renderEngine, recipe[9], x + 72, y + 18);
            RenderItem.getInstance()
                    .renderItemOverlayIntoGUI(mc.fontRenderer, mc.renderEngine, recipe[9], x + 72, y + 18);
        ...
        if (setRecipe && !canCraft(player, recipe, getFakeInv(player))) {
            ...
            mc.fontRenderer.drawStringWithShadow(
                    warning,
                    resolution.getScaledWidth() / 2 - mc.fontRenderer.getStringWidth(warning) / 2,
                    resolution.getScaledHeight() / 2 - yoff,
                    0xFFFFFF);
            yoff += 12;
        }
        ...
        mc.fontRenderer.drawStringWithShadow(
                label,
                resolution.getScaledWidth() / 2 - mc.fontRenderer.getStringWidth(label) / 2,
                resolution.getScaledHeight() / 2 - yoff,
                0xFFFFFF);
    }
