...
public class ItemGaiaHead extends ItemMod {
    ...
    public boolean onItemUse(
        ...
        if (sideDir == ForgeDirection.DOWN) return false;

        // If the side we're trying to place the skull on isn't solid, then
        // we can't place it either.
        if (!world.isSideSolid(x, y, z, sideDir)) return false;

        // Figure out where the skull actually goes based on the side we're placing it against.
        switch (sideDir) {
            case UP:
                y++;
                break; // If we're placing it on the top, then the skull goes 1 block above.
            case NORTH:
                z--;
                break; // Placing it on the north side (Z- axis).
            case SOUTH:
                z++;
                break; // Placing it on the south side (Z+ axis).
            case WEST:
                x--;
                break; // Placing it on the west side (X- axis).
            case EAST:
                x++;
                break; // Placing it on the east side (X+ axis).
            default:
                return false; // Oops, this shouldn't happen.
        }
        ...
        if (!ModBlocks.gaiaHead.canPlaceBlockOnSide(world, x, y, z, side)) return false;
