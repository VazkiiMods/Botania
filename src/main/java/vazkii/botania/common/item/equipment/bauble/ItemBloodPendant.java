...
public class ItemBloodPendant extends ItemBauble implements IBrewContainer, IBrewItem, IManaUsingItem, IBaubleRender {
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        ...
        for (String s : BotaniaAPI.brewMap.keySet()) {
            ...
            if (brewStack != null) list.add(brewStack);
        }
    }
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        icons = new IIcon[4];
        for (int i = 0; i < 4; i++) icons[i] = IconHelper.forItem(par1IconRegister, this, i);
    }
    ...
    public int getColorFromItemStack(ItemStack stack, int pass) {
        ...
        if (brew == BotaniaAPI.fallbackBrew) return 0xC6000E;
    ...
    public void addHiddenTooltip(ItemStack stack, EntityPlayer player, List list, boolean adv) {
        ...
        if (brew == BotaniaAPI.fallbackBrew) {
            addStringToTooltip(
                    EnumChatFormatting.LIGHT_PURPLE + StatCollector.translateToLocal("botaniamisc.notInfused"), list);
            return;
        }
        ...
        addStringToTooltip(
                EnumChatFormatting.LIGHT_PURPLE
                        + String.format(
                                StatCollector.translateToLocal("botaniamisc.brewOf"),
                                StatCollector.translateToLocal(brew.getUnlocalizedName(stack))),
                list);
        for (PotionEffect effect : brew.getPotionEffects(stack)) {
            ...
            addStringToTooltip(
                    " " + format + StatCollector.translateToLocal(effect.getEffectName())
                            + (effect.getAmplifier() == 0
                                    ? ""
                                    : " "
                                            + StatCollector.translateToLocal(
                                                    "botania.roman" + (effect.getAmplifier() + 1))),
                    list);
        }
    }
        ...
        if (brew != BotaniaAPI.fallbackBrew && player instanceof EntityPlayer && !player.worldObj.isRemote) {
            ...
            if (ManaItemHandler.requestManaExact(stack, eplayer, (int) Math.ceil(cost), false)) {
                ...
                if (currentEffect == null || currentEffect.getDuration() < (nightVision ? 205 : 3)) {
                    PotionEffect applyEffect =
                            new PotionEffect(effect.getPotionID(), nightVision ? 285 : 80, effect.getAmplifier(), true);
    ...
    public ItemStack getItemForBrew(Brew brew, ItemStack stack) {
        if (!brew.canInfuseBloodPendant()
                || brew.getPotionEffects(stack).size() != 1
                || Potion.potionTypes[brew.getPotionEffects(stack).get(0).getPotionID()].isInstant()) return null;
