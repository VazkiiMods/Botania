...
public class ItemFlightTiara extends ItemBauble implements IManaUsingItem, IBaubleRender, ICraftAchievement {
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < WING_TYPES; i++) wingIcons[i] = IconHelper.forItem(par1IconRegister, this, i + 1);
    }

    @Override
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < SUBTYPES + 1; i++) list.add(new ItemStack(item, 1, i));
    }
    ...
    public void onEquipped(ItemStack stack, EntityLivingBase player) {
        ...
        if (stack.getItemDamage() != WING_TYPES
                && hash(stack.getDisplayName())
                        .equals("16E1BDFD1D6AE1A954C9C5E1B2D9099780F3E1724541F1F2F77310B769CFFBAC")) {
    ...
    public void onWornTick(ItemStack stack, EntityLivingBase player) {
        ...
        if (player instanceof EntityPlayer) {
            ...
            if (isSprinting != wasSprting) ItemNBTHelper.setBoolean(stack, TAG_IS_SPRINTING, isSprinting);
            ...
            if (flying) {
                if (time > 0 && !ItemNBTHelper.getBoolean(stack, TAG_INFINITE_FLIGHT, false)) newTime--;
                final int maxCd = 80;
                ...
                if (!wasSprting && isSprinting && cooldown == 0) {
                    ...
                    if (maxCd - cooldown < 2) player.moveFlying(0F, 1F, 5F);
                    else if (maxCd - cooldown < 10) player.setSprinting(false);
                ...
                if (time < MAX_FLY_TIME && player.ticksExisted % (doGlide ? 6 : 2) == 0) newTime++;
            ...
            if (newTime != time) ItemNBTHelper.setInt(stack, TAG_TIME_LEFT, newTime);
        }
    }
    ...
    private boolean shouldPlayerHaveFlight(EntityPlayer player) {
        ...
        if (armor != null && armor.getItem() == this) {
            ...
            return (left > (flying ? 0 : MAX_FLY_TIME / 10) || player.inventory.hasItem(ModItems.flugelEye))
    ...
    public void onPlayerBaubleRender(ItemStack stack, RenderPlayerEvent event, RenderType type) {
        ...
        if (type == RenderType.BODY) {
            if (meta > 0 && meta <= ItemFlightTiara.wingIcons.length) {
                ...
                float rz = 120F;
                float rx = 20F
                        + (float) ((Math.sin((double) (event.entityPlayer.ticksExisted + event.partialRenderTick)
                                                * (flying ? 0.4F : 0.2F))
                                        + 0.5F)
                                * (flying ? 30F : 5F));
                float ry = 0F;
                ...
                switch (meta) {
                    ...
                    case 4: { // Phoenix
                        ...
                        h = 0.5F;
                        rx = 20F;
                        ry = -(float) ((Math.sin((double) (event.entityPlayer.ticksExisted + event.partialRenderTick)
                                                * (flying ? 0.4F : 0.2F))
                                        + 0.6F)
                                * (flying ? 30F : 5F));
                    ...
                    case 7: { // Lyfa
                        ...
                        GL11.glColor4f(
                                ...
                                1F,
                                0.5F
                                        + (float) Math.cos((double) (event.entityPlayer.ticksExisted
                ...
                ItemRenderer.renderItemIn2D(
                        Tessellator.instance, f1, f2, f, f3, icon.getIconWidth(), icon.getIconHeight(), 1F / 32F);
    ...
    public static void renderHalo(EntityPlayer player, float partialTicks) {
        ...
        if (player != null) Helper.translateToHeadLevel(player);
        GL11.glRotated(30, 1, 0, -1);
        GL11.glTranslatef(-0.1F, -0.5F, -0.1F);
        if (player != null) GL11.glRotatef(player.ticksExisted + partialTicks, 0, 1, 0);
    ...
    public static void renderHUD(ScaledResolution resolution, EntityPlayer player, ItemStack stack) {
        ...
        if (player.getAir() < 300) y = resolution.getScaledHeight() - ConfigHandler.flightBarBreathHeight;
        ...
        if (player.capabilities.isFlying) {
            ...
            GL11.glColor4f(1F, 1F, 1F, 1F);
            if (width > 0) Gui.drawRect(xo, y - 2, xo + 80, y - 1, 0x88000000);
