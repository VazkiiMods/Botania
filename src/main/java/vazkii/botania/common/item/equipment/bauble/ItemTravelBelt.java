...
public class ItemTravelBelt extends ItemBauble implements IBaubleRender, IManaUsingItem {
        ...
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (playersWithStepup.contains(s)) {
                if (shouldPlayerHaveStepup(player)) {
                    ...
                    if (player.isSneaking()) player.stepHeight = 0.50001F; // Not 0.5F because that is the default
    ...
    public void onPlayerJump(LivingJumpEvent event) {
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (belt != null
                    && belt.getItem() instanceof ItemTravelBelt
                    && ManaItemHandler.requestManaExact(belt, player, COST, false)) {
    ...
    private boolean shouldPlayerHaveStepup(EntityPlayer player) {
        ...
        return armor != null
                && armor.getItem() instanceof ItemTravelBelt
                && ManaItemHandler.requestManaExact(armor, player, COST, false);
    }
