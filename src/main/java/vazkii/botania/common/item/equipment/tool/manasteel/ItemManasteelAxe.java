...
public class ItemManasteelAxe extends ItemAxe implements IManaUsingItem, ISortableTool {
    ...
    private static final Pattern SAPLING_PATTERN =
            Pattern.compile("(?:(?:(?:[A-Z-_.:]|^)sapling)|(?:(?:[a-z-_.:]|^)Sapling))(?:[A-Z-_.:]|$)");
    ...
    public boolean onBlockDestroyed(
        ...
        if (block.getBlockHardness(world, x, y, z) != 0F) ToolCommons.damageItem(stack, 1, entity, getManaPerDamage());
    ...
    public boolean onItemUse(
        ...
        for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
            ...
            if (stackAt != null
                    && SAPLING_PATTERN
                            .matcher(stackAt.getItem().getUnlocalizedName())
                            .find()) {
    ...
    public void onUpdate(ItemStack stack, World world, Entity player, int par4, boolean par5) {
        if (!world.isRemote
                ...
                && ManaItemHandler.requestManaExactForTool(stack, (EntityPlayer) player, getManaPerDamage() * 2, true))
