...
public class ItemMagnetRing extends ItemBauble {
    ...
    public void onWornTick(ItemStack stack, EntityLivingBase player) {
        ...
        if (SubTileSolegnolia.hasSolegnoliaAround(player)) {
            if (cooldown < 0) setCooldown(stack, 2);
            return;
        }
    ...
    private boolean canPullItem(EntityItem item) {
        if (item.isDead || SubTileSolegnolia.hasSolegnoliaAround(item)) return false;

        ItemStack stack = item.getEntityItem();
        if (stack == null
                || stack.getItem() instanceof IManaItem
                || stack.getItem() instanceof IRelic
                || BLACKLIST.contains(itemRegistry.getNameForObject(stack.getItem()))
                || BotaniaAPI.isItemBlacklistedFromMagnet(stack)) return false;
        ...
        if (BotaniaAPI.isBlockBlacklistedFromMagnet(block, meta)) return false;
