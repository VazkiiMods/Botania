...
public class ItemGlassPick extends ItemManasteelPick {
    ...
    public void onBlockDrops(HarvestDropsEvent event) {
        if (event.harvester != null
                ...
                && event.harvester.getCurrentEquippedItem().getItem() == this
                && event.block.getMaterial() == Material.glass
                && event.block.canSilkHarvest(
                        event.world, event.harvester, event.x, event.y, event.z, event.blockMetadata))
    ...
    public boolean getIsRepairable(ItemStack par1ItemStack, ItemStack par2ItemStack) {
        return par2ItemStack.getItem() == Item.getItemFromBlock(Blocks.glass)
                ? true
