...
public class ItemIcePendant extends ItemBauble implements IBaubleRender {
    ...
    public void onWornTick(ItemStack stack, EntityLivingBase entity) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (!player.worldObj.isRemote) tickIceRemovers(player);
    ...
    private void addIceBlock(EntityPlayer player, ChunkCoordinates coords) {
        ...
        if (!playerIceBlocks.containsKey(user)) playerIceBlocks.put(user, new ArrayList());

        List<IceRemover> ice = playerIceBlocks.get(user);
        if (player.worldObj.getBlock(coords.posX, coords.posY, coords.posZ) == Blocks.water
                && player.worldObj.getBlockMetadata(coords.posX, coords.posY, coords.posZ) == 0) {
            player.worldObj.setBlock(coords.posX, coords.posY, coords.posZ, Blocks.ice);

            if (!player.worldObj.isRemote) ice.add(new IceRemover(coords));
        }
    }

    private void tickIceRemovers(EntityPlayer player) {
        String user = player.getCommandSenderName();
        if (!playerIceBlocks.containsKey(user)) return;

        List<IceRemover> removers = playerIceBlocks.get(user);
        for (IceRemover ice : new ArrayList<IceRemover>(removers)) ice.tick(player.worldObj, removers);
    }
    ...
    public void onPlayerBaubleRender(ItemStack stack, RenderPlayerEvent event, RenderType type) {
        if (type == RenderType.BODY) {
            ...
            ItemRenderer.renderItemIn2D(
                    Tessellator.instance, f1, f2, f, f3, gemIcon.getIconWidth(), gemIcon.getIconHeight(), 1F / 32F);
        }
    }
    ...
    class IceRemover {
        ...
        public void tick(World world, List<IceRemover> list) {
            if (world.getBlock(coords.posX, coords.posY, coords.posZ) == Blocks.ice) {
                if (time-- == 0) world.setBlock(coords.posX, coords.posY, coords.posZ, Blocks.water, 0, 1 | 2);
                else return;
                list.remove(this);
            }
        }
