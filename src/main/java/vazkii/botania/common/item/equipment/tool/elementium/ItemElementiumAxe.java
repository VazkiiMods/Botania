        ...
        if (event.recentlyHit && event.source.getEntity() != null && event.source.getEntity() instanceof EntityPlayer) {
            ...
            if (weapon != null && weapon.getItem() == this) {
                ...
                if (event.entityLiving instanceof EntitySkeleton && rand.nextInt(26) <= 3 + looting)
                    addDrop(
                            event,
                            new ItemStack(Items.skull, 1, ((EntitySkeleton) event.entityLiving).getSkeletonType()));
                else if (event.entityLiving instanceof EntityZombie
                        && !(event.entityLiving instanceof EntityPigZombie)
                        && rand.nextInt(26) <= 2 + 2 * looting) addDrop(event, new ItemStack(Items.skull, 1, 2));
                    ...
                    ItemNBTHelper.setString(
                            stack, "SkullOwner", ((EntityPlayer) event.entityLiving).getCommandSenderName());
                    addDrop(event, stack);
