...
public class ItemSwapRing extends ItemBauble {
    ...
    public void onWornTick(ItemStack stack, EntityLivingBase entity) {
        ...
        if (currentStack == null || !(currentStack.getItem() instanceof ISortableTool)) return;
        ...
        if (player.isSwingInProgress && pos != null) {
            ...
            if (block != null) {
                ...
                if (ToolCommons.isRightMaterial(mat, ToolCommons.materialsPick)) typeToFind = ToolType.PICK;
                else if (ToolCommons.isRightMaterial(mat, ToolCommons.materialsShovel)) typeToFind = ToolType.SHOVEL;
                else if (ToolCommons.isRightMaterial(mat, ToolCommons.materialsAxe)) typeToFind = ToolType.AXE;
            }
        }

        if (typeToFind == null) return;

        ItemStack bestTool = currentStack;
        int bestToolPriority =
                tool.getSortingType(currentStack) == typeToFind ? tool.getSortingPriority(currentStack) : -1;
        int bestSlot = -1;
