...
public class ItemElementiumShears extends ItemManasteelShears {
    ...
    public IIcon getIconIndex(ItemStack par1ItemStack) {
        return par1ItemStack.getDisplayName().equalsIgnoreCase("dammit reddit")
                ? dammitReddit
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        if (count != getMaxItemUseDuration(stack) && count % 5 == 0) {
            int range = 12;
            List<IShearable> sheep = player.worldObj.getEntitiesWithinAABB(
                    IShearable.class,
            ...
            if (sheep.size() > 0) {
                for (IShearable target : sheep) {
                    ...
                    if (target.isShearable(
                            stack, entity.worldObj, (int) entity.posX, (int) entity.posY, (int) entity.posZ)) {
