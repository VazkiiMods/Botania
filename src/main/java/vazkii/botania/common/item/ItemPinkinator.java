...
public class ItemPinkinator extends ItemMod {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        int range = 16;
        List<EntityWither> withers = world.getEntitiesWithinAABB(
                EntityWither.class,
        ...
        for (EntityWither wither : withers)
            if (!wither.isDead && !(wither instanceof EntityPinkWither)) {
                if (!world.isRemote) {
                    ...
                    pink.setLocationAndAngles(
                            wither.posX, wither.posY, wither.posZ, wither.rotationYaw, wither.rotationPitch);
