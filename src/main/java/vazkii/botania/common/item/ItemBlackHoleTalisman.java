...
public class ItemBlackHoleTalisman extends ItemMod implements IBlockProvider {
    ...
    public ItemBlackHoleTalisman() {
        ...
        RecipeSorter.register(
                "botania:blackHoleTalismanExtract", BlackHoleTalismanExtractRecipe.class, Category.SHAPELESS, "");
    }
                ...
                if (entities == 0) {
                    ...
                    if (remove > 0) {
                        ...
                        Item.getItemFromBlock(bBlock)
                                .onItemUse(
                                        stack, par2EntityPlayer, par3World, par4, par5, par6, par7, par8, par9, par10);
                        set = true;
                    }
                }
            }
    ...
    public boolean provideBlock(
        ...
        if (stored == block && storedMeta == meta) {
            ...
            if (count > 0) {
                if (doit) setCount(stack, count - 1);
                return true;
            }
        }
    ...
    public int getBlockCount(EntityPlayer player, ItemStack requestor, ItemStack stack, Block block, int meta) {
        Block stored = getBlock(stack);
        int storedMeta = getBlockMeta(stack);
        if (stored == block && storedMeta == meta) return getBlockCount(stack);
        return 0;
    }
}
