...
public class LensFlare extends Lens {
    ...
    public void onControlledSpreaderTick(ItemStack stack, IManaSpreader spreader, boolean redstone) {
        if (!redstone) emitParticles(stack, spreader, redstone);
    }
    ...
    private void emitParticles(ItemStack stack, IManaSpreader spreader, boolean redstone) {
        ...
        float f = 0.3F;
        float mx = (float) (MathHelper.sin(rotationYaw / 180.0F * (float) Math.PI)
                * MathHelper.cos(rotationPitch / 180.0F * (float) Math.PI)
                * f
                / 2D);
        float mz = (float) (-(MathHelper.cos(rotationYaw / 180.0F * (float) Math.PI)
                        * MathHelper.cos(rotationPitch / 180.0F * (float) Math.PI)
                        * f)
                / 2D);
        ...
        Botania.proxy.wispFX(
                tile.getWorldObj(), tile.xCoord + 0.5, tile.yCoord + 0.5, tile.zCoord + 0.5, r, g, b, 0.4F, mx, my, mz);
    }
}
