...
public class LensWeight extends Lens {
    ...
    public boolean collideBurst(
        ...
        if (!burst.isFake()) {
            ...
            if (blockBelow.isAir(entity.worldObj, x, y - 1, z)
                    && block.getBlockHardness(entity.worldObj, x, y, z) != -1
                    && neededHarvestLevel <= harvestLevel
                    && entity.worldObj.getTileEntity(x, y, z) == null
                    && block.canSilkHarvest(entity.worldObj, null, x, y, z, meta)) {
                EntityFallingBlock falling =
                        new EntityFallingBlock(entity.worldObj, x + 0.5, y + 0.5, z + 0.5, block, meta);
                if (!entity.worldObj.isRemote) entity.worldObj.spawnEntityInWorld(falling);
            }
        }
