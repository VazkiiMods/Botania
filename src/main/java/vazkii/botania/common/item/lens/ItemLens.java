...
public class ItemLens extends ItemMod implements ILensControl, ICompositableLens, ITinyPlanetExcempt {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (storedColor != -1)
            par3List.add(String.format(
                    StatCollector.translateToLocal("botaniamisc.color"),
                    StatCollector.translateToLocal("botania.color" + storedColor)));
    }

    public String getItemShortTermName(ItemStack stack) {
        return StatCollector.translateToLocal(
                stack.getUnlocalizedName().replaceAll("item.", "item.botania:") + ".short");
    }
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        ...
        return String.format(
                StatCollector.translateToLocal("item.botania:compositeLens.name"),
                getItemShortTermName(stack),
    ...
    public void updateBurst(IManaBurst burst, ItemStack stack) {
        ...
        if (storedColor == 16 && entity.worldObj.isRemote) burst.setColor(getLensColor(stack));
    ...
    public static Lens getLens(int index) {
        if (index == STORM) return stormLens;
    ...
    public boolean canCombineLenses(ItemStack sourceLens, ItemStack compositeLens) {
        ...
        if (sourceItem == compositeItem && sourceLens.getItemDamage() == compositeLens.getItemDamage()) return false;

        if (!sourceItem.isCombinable(sourceLens) || !compositeItem.isCombinable(compositeLens)) return false;

        if (isBlacklisted(sourceLens, compositeLens)) return false;
