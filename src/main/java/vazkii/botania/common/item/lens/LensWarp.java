...
public class LensWarp extends Lens {
    ...
    public boolean collideBurst(
            ...
            boolean dead,
            ItemStack stack) {
        if (burst.isFake()) return dead;

        Block block = entity.worldObj.getBlock(pos.blockX, pos.blockY, pos.blockZ);
        if (block == ModBlocks.pistonRelay) {
            String key = BlockPistonRelay.mappedPositions.get(BlockPistonRelay.getCoordsAsString(
                    entity.worldObj.provider.dimensionId, pos.blockX, pos.blockY, pos.blockZ));
            if (key != null) {
