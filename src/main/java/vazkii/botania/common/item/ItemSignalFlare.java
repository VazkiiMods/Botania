...
public class ItemSignalFlare extends ItemMod {
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        if (par1ItemStack.getItemDamage() == 0) {
            if (par2World.isRemote) par3EntityPlayer.swingItem();
                ...
                int stunned = 0;
                int range = 5;
                List<EntityLivingBase> entities = par2World.getEntitiesWithinAABB(
                        EntityLivingBase.class,
                        AxisAlignedBB.getBoundingBox(
                                par3EntityPlayer.posX - range,
                                par3EntityPlayer.posY - range,
                                par3EntityPlayer.posZ - range,
                                par3EntityPlayer.posX + range,
                                par3EntityPlayer.posY + range,
                                par3EntityPlayer.posZ + range));
                ...
                if (stunned >= 100) par3EntityPlayer.addStat(ModAchievements.signalFlareStun, 1);
            }
    ...
    public int getColorFromItemStack(ItemStack par1ItemStack, int par2) {
        ...
        if (colorv >= EntitySheep.fleeceColorTable.length || colorv < 0) return 0xFFFFFF;
    ...
    public void getSubItems(Item par1, CreativeTabs par2CreativeTabs, List par3List) {
        for (int i = 0; i < 16; i++) par3List.add(forColor(i));
    }
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        par3List.add(String.format(
                StatCollector.translateToLocal("botaniamisc.flareColor"),
