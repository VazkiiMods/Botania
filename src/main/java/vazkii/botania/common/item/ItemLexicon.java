...
public class ItemLexicon extends ItemMod implements ILexicon, IElvenItem {
            ...
            if (block != null) {
                if (block instanceof ILexiconable) {
                    LexiconEntry entry = ((ILexiconable) block)
                            .getEntry(par3World, par4, par5, par6, par2EntityPlayer, par1ItemStack);
                    ...
                    MovingObjectPosition pos = new MovingObjectPosition(
                            par4, par5, par6, par7, Vec3.createVectorHelper(par8, par9, par10));
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            String edition = EnumChatFormatting.GOLD
                    + String.format(StatCollector.translateToLocal("botaniamisc.edition"), getEdition());
            if (!edition.isEmpty()) par3List.add(edition);

            List<KnowledgeType> typesKnown = new ArrayList();
            for (String s : BotaniaAPI.knowledgeTypes.keySet()) {
                KnowledgeType type = BotaniaAPI.knowledgeTypes.get(s);
                if (isKnowledgeUnlocked(par1ItemStack, type)) typesKnown.add(type);
            }

            String format =
                    typesKnown.size() == 1 ? "botaniamisc.knowledgeTypesSingular" : "botaniamisc.knowledgeTypesPlural";
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        ...
        if (force != null && !force.isEmpty()) {
            ...
            if (entry != null) Botania.proxy.setEntryToOpen(entry);
            else
                par3EntityPlayer.addChatMessage(new ChatComponentTranslation("botaniamisc.cantOpen")
    ...
    public static void openBook(EntityPlayer player, ItemStack stack, World world, boolean skipSound) {
        ...
        if (!world.isRemote && !skipSound) world.playSoundAtEntity(player, "botania:lexiconOpen", 0.5F, 1F);
    }
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int idk, boolean something) {
        ...
        if (ticks > 0 && entity instanceof EntityPlayer) {
            skipSound = ticks < 5;
            if (ticks == 1) onItemRightClick(stack, world, (EntityPlayer) entity);
