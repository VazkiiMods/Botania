...
public class ItemEnderHand extends Item implements IManaUsingItem, IBlockProvider {
	...
	public boolean provideBlock(PlayerEntity player, ItemStack requestor, ItemStack stack, Block block, boolean doit) {
		...
		ItemStack istack = ItemExchangeRod.removeFromInventory(player, player.getInventoryEnderChest(), stack, block.asItem(), false);
		if (!istack.isEmpty()) {
			...
			if (mana) {
				if (doit) {
					...
					ItemExchangeRod.removeFromInventory(player, player.getInventoryEnderChest(), stack, block.asItem(), true);
				}
	...
	public int getBlockCount(PlayerEntity player, ItemStack requestor, ItemStack stack, Block block) {
		...
		return ItemExchangeRod.getInventoryItemCount(player, player.getInventoryEnderChest(), stack, block.asItem());
	}
