...
public class ItemLaputaShard extends ItemMod implements ILensEffect, ITinyPlanetExcempt {
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        ...
        for (int i = 0; i < 4; i++) list.add(new ItemStack(item, 1, (i + 1) * 5 - 1));
    }

    @Override
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean adv) {
        list.add(String.format(
                StatCollector.translateToLocal("botaniamisc.shardLevel"),
                StatCollector.translateToLocal("botania.roman" + (stack.getItemDamage() + 1))));
    }
    ...
    public boolean onItemUse(
        ...
        if (par5 < 160 && !par3World.provider.isHellWorld) {
            par3World.playSound(
                    ...
                    par6 + 0.5D,
                    "botania:laputaStart",
                    1.0F + par3World.rand.nextFloat(),
                    par3World.rand.nextFloat() * 0.7F + 1.3F,
                    false);
            spawnBurstFirst(par3World, par4, par5, par6, par1ItemStack);
            par1ItemStack.stackSize--;
            if (par1ItemStack.getItemDamage() == 19) par2EntityPlayer.addStat(ModAchievements.l20ShardUse, 1);
        }
    ...
    public void spawnBurst(
            World world, int srcx, int srcy, int srcz, ItemStack lens, boolean pointy, double heightscale) {
        int range = BASE_RANGE + lens.getItemDamage();
        ...
        if (j <= -BASE_RANGE * 2) j = BASE_OFFSET - BASE_RANGE / 2;
        if (k >= range * 2 + 1) k = 0;
                    ...
                    for (; k < range * 2 + 1; k++) {
                        ...
                        if (inRange(x, y, z, srcx, srcy, srcz, range, heightscale, pointy)) {
                            ...
                            if (!block.isAir(world, x, y, z)
                                    ...
                                    && (!(block instanceof ILaputaImmobile)
                                            || ((ILaputaImmobile) block).canMove(world, x, y, z))
    ...
    private boolean inRange(
            int x, int y, int z, int srcx, int srcy, int srcz, int range, double heightscale, boolean pointy) {
        if (y >= srcy) return MathHelper.pointDistanceSpace(x, 0, z, srcx, 0, srcz) < range;
        else if (!pointy)
    ...
    public void updateBurst(IManaBurst burst, ItemStack stack) {
        ...
        if (!entity.worldObj.isRemote) {
            ...
            if (burst.getTicksExisted() == spawnTicks) {
                ...
                if (y != -1) spawnBurst(entity.worldObj, x, y, z, lens);
                ...
                if (entity.worldObj.isAirBlock(x, y, z)) {
                    ...
                    if (tilecmp.hasKey("id")) tile = TileEntity.createAndLoadEntity(tilecmp);
