...
public abstract class ItemBrewBase extends ItemMod implements IBrewItem, IPickupAchievement {
        ...
        if (!world.isRemote) {
            for (PotionEffect effect : getBrew(stack).getPotionEffects(stack)) {
                PotionEffect newEffect =
                        new PotionEffect(effect.getPotionID(), effect.getDuration(), effect.getAmplifier(), true);
                Potion potion = Potion.potionTypes[newEffect.getPotionID()];
                if (potion.isInstant()) potion.affectEntity(player, player, newEffect.getAmplifier(), 1F);
                else player.addPotionEffect(newEffect);
            }

            if (world.rand.nextBoolean()) world.playSoundAtEntity(player, "random.burp", 1F, 1F);

            int swigs = getSwigsLeft(stack);
            if (!player.capabilities.isCreativeMode) {
                if (swigs == 1) {
                    ItemStack copy = baseItem.copy();
                    if (!player.inventory.addItemStackToInventory(copy)) return baseItem.copy();
                    return null;
                }
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < swigs; i++) icons[i] = IconHelper.forName(par1IconRegister, texName + "1_" + i);
    }
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        return String.format(
                StatCollector.translateToLocal(getUnlocalizedNameInefficiently(stack) + ".name"),
                StatCollector.translateToLocal(getBrew(stack).getUnlocalizedName(stack)),
                EnumChatFormatting.BOLD + "" + getSwigsLeft(stack) + EnumChatFormatting.RESET);
    }
        ...
        for (PotionEffect effect : brew.getPotionEffects(stack)) {
            ...
            list.add(format
                    ...
                    + (effect.getAmplifier() == 0
                            ? ""
                            : " " + StatCollector.translateToLocal("botania.roman" + (effect.getAmplifier() + 1)))
                    + EnumChatFormatting.GRAY
                    + (potion.isInstant() ? "" : " (" + Potion.getDurationString(effect) + ")"));
        }
    }
