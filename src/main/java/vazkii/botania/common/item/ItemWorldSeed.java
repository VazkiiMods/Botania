...
public class ItemWorldSeed extends ItemMod {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (MathHelper.pointDistanceSpace(
                        coords.posX + 0.5, coords.posY + 0.5, coords.posZ + 0.5, player.posX, player.posY, player.posZ)
                > 24) {
            ...
            for (int i = 0; i < 50; i++)
                Botania.proxy.sparkleFX(
                        world,
                        player.posX + Math.random() * player.width,
                        player.posY - 1.6 + Math.random() * player.height,
                        player.posZ + Math.random() * player.width,
                        0.25F,
                        1F,
                        0.25F,
                        1F,
