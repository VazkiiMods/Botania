...
public class ItemCacophonium extends ItemMod implements ICraftAchievement {
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase entity) {
        if (entity instanceof EntityLiving) {
            ...
            String sound = null;
            try {
                if (living instanceof EntityCreeper) sound = "creeper.primed";
                else if (living instanceof EntitySlime)
                    sound = "mob.slime." + (((EntitySlime) living).getSlimeSize() > 1 ? "big" : "small");
                else
                    sound = (String)
                            ReflectionHelper.findMethod(EntityLiving.class, living, LibObfuscation.GET_LIVING_SOUND)
                                    .invoke(living);
    ...
    public boolean onItemUse(
        ...
        if (!can) {
            ...
            isDOIT(stack);
            if (sound != null && !sound.isEmpty()) can = true;
        }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean adv) {
        if (isDOIT(stack)) list.add(StatCollector.translateToLocal("botaniamisc.justDoIt"));
    ...
    public static void playSound(World world, ItemStack stack, double x, double y, double z, float volume) {
        ...
        if (sound != null && !sound.isEmpty())
            world.playSoundEffect(
                    ...
                    sound,
                    volume,
                    doit ? 1F : (world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F + 1.0F);
    }
