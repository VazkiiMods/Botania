...
public class ItemVirus extends ItemMod {
        ...
        if (par3EntityLivingBase instanceof EntityHorse) {
            ...
            if (horse.getHorseType() != 3 && horse.getHorseType() != 4 && horse.isTame()) {
                ...
                IAttributeInstance movementSpeed =
                        attributes.getAttributeInstance(SharedMonsterAttributes.movementSpeed);
                ...
                movementSpeed.applyModifier(
                        new AttributeModifier("Ermergerd Virus D:", movementSpeed.getBaseValue(), 0));
                IAttributeInstance jumpHeight =
                        attributes.getAttributeInstance(ReflectionHelper.<IAttribute, EntityHorse>getPrivateValue(
                                EntityHorse.class, null, LibObfuscation.HORSE_JUMP_STRENGTH));
                jumpHeight.applyModifier(
                        new AttributeModifier("Ermergerd Virus D:", jumpHeight.getBaseValue() * 0.5, 0));
                par2EntityPlayer.worldObj.playSound(
                        par3EntityLivingBase.posX + 0.5D,
                        par3EntityLivingBase.posY + 0.5D,
                        par3EntityLivingBase.posZ + 0.5D,
                        "mob.zombie.remedy",
                        1.0F + par3EntityLivingBase.worldObj.rand.nextFloat(),
                        par3EntityLivingBase.worldObj.rand.nextFloat() * 0.7F + 1.3F,
                        false);
    ...
    public void onLivingHurt(LivingHurtEvent event) {
        ...
        if (entity instanceof EntityHorse && event.source == DamageSource.fall) {
            ...
            if ((horse.getHorseType() == 3 || horse.getHorseType() == 4) && horse.isTame()) event.setCanceled(true);
        }
    }
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < SUBTYPES; i++) list.add(new ItemStack(item, 1, i));
    }
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < SUBTYPES; i++) icons[i] = IconHelper.forItem(par1IconRegister, this, i);
    }
}
