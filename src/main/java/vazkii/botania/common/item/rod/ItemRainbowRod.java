...
public class ItemRainbowRod extends ItemMod implements IManaUsingItem, IAvatarWieldable {
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        if (!par2World.isRemote
            ...
            while (count < maxlen && (int) lx == (int) x && (int) ly == (int) y && (int) lz == (int) z
                    || count < 4
                    || par2World.getBlock((int) x, (int) y, (int) z).isAir(par2World, (int) x, (int) y, (int) z)
                    || par2World.getBlock((int) x, (int) y, (int) z) == place) {
                if (y >= 256 || y <= 0) break;

                for (int i = -2; i < 1; i++)
                    for (int j = -2; j < 1; j++)
                        if (par2World
                                        .getBlock((int) x + i, (int) y, (int) z + j)
                                        .isAir(par2World, (int) x + i, (int) y, (int) z + j)
                                || par2World.getBlock((int) x + i, (int) y, (int) z + j) == place) {
                            ...
                            if (tile != null) {
                                for (int k = 0; k < 4; k++)
                                    Botania.proxy.sparkleFX(
                                            par2World,
                                            tile.xCoord + Math.random(),
                                            tile.yCoord + Math.random(),
                                            tile.zCoord + Math.random(),
                                            (float) Math.random(),
                                            (float) Math.random(),
                                            (float) Math.random(),
                                            0.45F + 0.2F * (float) Math.random(),
                                            6);
                                tile.ticks = time;
                            }
                        }
    ...
    public void onAvatarUpdate(IAvatarTile tile, ItemStack stack) {
        ...
        if (world.isRemote || tile.getCurrentMana() < MANA_COST_AVATAR * 25 || !tile.isEnabled()) return;
        ...
        for (EntityPlayer p : players) {
            ...
            for (int i = 0; i < dist; i++)
                for (int j = 0; j < dist; j++) {
                    ...
                    if (!axis.isVecInside(Vec3.createVectorHelper(ex + 0.5, py + 1, ez + 0.5))) continue;
