...
public class ItemGravityRod extends ItemMod implements IManaUsingItem {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (ticksCooldown == 0) {
            ...
            if (item != null) {
                if (BotaniaAPI.isEntityBlacklistedFromGravityRod(item.getClass())) return stack;
                ...
                if (found) {
                    ...
                    if (item instanceof EntityItem) {
                        ...
                        if (!player.worldObj.isRemote) {
                            EntityThrownItem thrown = new EntityThrownItem(
                                    item.worldObj, item.posX, item.posY, item.posZ, (EntityItem) item);
