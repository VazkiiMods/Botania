...
public class ItemExchangeRod extends ItemMod implements IManaUsingItem, IWireframeCoordinateListProvider {
        ...
        if (par2EntityPlayer.isSneaking()) {
            ...
            if (tile == null) {
                if (BlockCamo.isValidBlock(wblock)) {
                    ...
                    if (!item.getHasSubtypes()) wmeta = 0;
            ...
            List<ChunkCoordinates> swap =
                    getBlocksToSwap(par3World, par1ItemStack, block, meta, par4, par5, par6, null, 0);
            if (swap.size() > 0) {
    ...
    public void onLeftClick(PlayerInteractEvent event) {
        if (event.action == Action.LEFT_CLICK_BLOCK) {
            ...
            if (stack != null
                    && stack.getItem() == this
                    && canExchange(stack)
                    && ManaItemHandler.requestManaExactForTool(stack, event.entityPlayer, COST, false)) {
                if (exchange(
                        event.world,
                        event.entityPlayer,
                        event.x,
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int something, boolean somethingelse) {
        if (!canExchange(stack) || !(entity instanceof EntityPlayer)) return;
        ...
        if (extraRange != extraRangeNew) ItemNBTHelper.setInt(stack, TAG_EXTRA_RANGE, extraRangeNew);
        ...
        if (ItemNBTHelper.getBoolean(stack, TAG_SWAPPING, false)) {
            ...
            if (exchange) ManaItemHandler.requestManaExactForTool(stack, player, COST, true);
            else ItemNBTHelper.setBoolean(stack, TAG_SWAPPING, false);
        }
    }

    public List<ChunkCoordinates> getBlocksToSwap(
            World world,
            ItemStack stack,
            Block blockToSwap,
            int metaToSwap,
        ...
        for (int offsetX = -effRange; offsetX <= effRange; offsetX++)
            for (int offsetY = -effRange; offsetY <= effRange; offsetY++)
                for (int offsetZ = -effRange; offsetZ <= effRange; offsetZ++) {
                    ...
                    if (currentBlock == blockToSwap && currentMeta == metaToSwap) continue;
    ...
    public boolean exchange(
            World world, EntityPlayer player, int x, int y, int z, ItemStack stack, Block blockToSet, int metaToSet) {
    ...
    public static ItemStack removeFromInventory(
            EntityPlayer player, IInventory inv, ItemStack stack, Block block, int meta, boolean doit) {
        ...
        for (int i = inv.getSizeInventory() - 1; i >= 0; i--) {
            ...
            if (item instanceof IBlockProvider) providers.add(invStack);
        }

        for (ItemStack provStack : providers) {
            IBlockProvider prov = (IBlockProvider) provStack.getItem();
            if (prov.provideBlock(player, stack, provStack, block, meta, doit)) return new ItemStack(block, 1, meta);
        }
    ...
    public static ItemStack removeFromInventory(
            EntityPlayer player, ItemStack stack, Block block, int meta, boolean doit) {
        if (player.capabilities.isCreativeMode) return new ItemStack(block, 1, meta);

        ItemStack outStack = removeFromInventory(
                player, BotaniaAPI.internalHandler.getBaublesInventory(player), stack, block, meta, doit);
        if (outStack == null) outStack = removeFromInventory(player, player.inventory, stack, block, meta, doit);
        return outStack;
    }
    ...
    public static int getInventoryItemCount(EntityPlayer player, ItemStack stack, Block block, int meta) {
        ...
        int baubleCount = getInventoryItemCount(
                player, BotaniaAPI.internalHandler.getBaublesInventory(player), stack, block, meta);
    ...
    public static int getInventoryItemCount(
            EntityPlayer player, IInventory inv, ItemStack stack, Block block, int meta) {
        if (player.capabilities.isCreativeMode) return -1;
        ...
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            ...
            Item item = invStack.getItem();
            if (item == Item.getItemFromBlock(block) && invStack.getItemDamage() == meta) count += invStack.stackSize;
    ...
    public String getItemStackDisplayName(ItemStack par1ItemStack) {
        ...
        return super.getItemStackDisplayName(par1ItemStack)
                + (block == null
                        ? ""
                        : " (" + EnumChatFormatting.GREEN + new ItemStack(block, 1, meta).getDisplayName()
    ...
    public List<ChunkCoordinates> getWireframesToDraw(EntityPlayer player, ItemStack stack) {
        ...
        if (holding != stack || !canExchange(stack)) return null;
        ...
        if (pos != null) {
            ...
            if (!player.worldObj.isAirBlock(x, y, z)) {
                List<ChunkCoordinates> coordsList =
                        getBlocksToSwap(player.worldObj, stack, block, meta, x, y, z, targetBlock, targetMeta);
