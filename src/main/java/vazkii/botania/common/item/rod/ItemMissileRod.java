...
public class ItemMissileRod extends ItemMod implements IManaUsingItem, IAvatarWieldable {
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        if (count != getMaxItemUseDuration(stack)
                && count % (IManaProficiencyArmor.Helper.hasProficiency(player) ? 1 : 2) == 0
                && !player.worldObj.isRemote
                && ManaItemHandler.requestManaExactForTool(stack, player, COST_PER, false)) {
            if (spawnMissile(
                    player.worldObj,
                    player,
                    player.posX + (Math.random() - 0.5 * 0.1),
                    player.posY + 2.4 + (Math.random() - 0.5 * 0.1),
                    player.posZ + (Math.random() - 0.5 * 0.1)))
    ...
    public boolean spawnMissile(World world, EntityLivingBase thrower, double x, double y, double z) {
        ...
        if (thrower != null) missile = new EntityMagicMissile(thrower, false);
    ...
    public void onAvatarUpdate(IAvatarTile tile, ItemStack stack) {
        ...
        if (tile.getCurrentMana() >= COST_AVATAR && tile.getElapsedFunctionalTicks() % 3 == 0 && tile.isEnabled())
            if (spawnMissile(
                    world,
                    null,
                    te.xCoord + 0.5 + (Math.random() - 0.5 * 0.1),
                    te.yCoord + 2.5 + (Math.random() - 0.5 * 0.1),
                    te.zCoord + (Math.random() - 0.5 * 0.1))) {
