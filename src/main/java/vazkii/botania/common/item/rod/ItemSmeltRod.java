...
public class ItemSmeltRod extends ItemMod implements IManaUsingItem {
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer p, int time) {
        if (!ManaItemHandler.requestManaExactForTool(stack, p, COST_PER_TICK, false)) return;
            ...
            if (result != null && result.getItem() instanceof ItemBlock) {
                        ...
                        if (data.progress <= 0) {
                            ...
                            for (int i = 0; i < 25; i++) {
                                ...
                                Botania.proxy.wispFX(
                                        p.worldObj, x, y, z, 1F, 0.2F, 0.2F, 0.5F, (float) -Math.random() / 10F);
                            }
                        }
                    }
                }
                ...
                if (!decremented)
                    playerData.put(
                            p,
                            new SmeltData(
                                    pos, IManaProficiencyArmor.Helper.hasProficiency(p) ? (int) (TIME * 0.6) : TIME));
                else {
                    for (int i = 0; i < 2; i++) {
                        double x = pos.blockX + Math.random();
                        double y = pos.blockY + Math.random();
