...
public class ItemTerraformRod extends ItemMod implements IManaUsingItem, IBlockProvider, ICraftAchievement {
    ...
    public void terraform(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
                    ...
                    for (int id : ids)
                        if (validBlocks.contains(OreDictionary.getOreName(id))) {
                            ...
                            for (ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS) {
                                ...
                                if (block_.isAir(par2World, x_, y_, z_)
                                        || block_.isReplaceable(par2World, x_, y_, z_)
                                ...
                                if (y > yCenter) blocks.add(new CoordsWithBlock(x, y, z, Blocks.air));
                                    ...
                                    for (ChunkCoordinates coords : airBlocks) {
                                        if (par2World.getBlock(coords.posX, coords.posY - 1, coords.posZ) != Blocks.air)
                                            blocks.add(new CoordsWithBlock(
                                                    coords.posX, coords.posY, coords.posZ, Blocks.dirt));
                                    }
                            }
                            break;
                        }
        ...
        if (par2World.isRemote
            ...
            if (!blocks.isEmpty()) {
                for (int i = 0; i < 10; i++) par2World.playSoundAtEntity(par3EntityPlayer, "step.sand", 1F, 0.4F);
                for (int i = 0; i < 120; i++)
                    Botania.proxy.sparkleFX(
                            par2World,
                            xCenter - range + range * 2 * Math.random(),
                            yCenter + 2 + (Math.random() - 0.5) * 2,
                            zCenter - range + range * 2 * Math.random(),
                            0.35F,
                            0.2F,
                            0.05F,
                            2F,
    ...
    public int getBlockCount(EntityPlayer player, ItemStack requestor, ItemStack stack, Block block, int meta) {
        if (block == Blocks.dirt && meta == 0) return -1;
        return 0;
    }
