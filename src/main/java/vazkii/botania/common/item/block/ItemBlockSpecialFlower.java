...
public class ItemBlockSpecialFlower extends ItemBlockMod implements IRecipeKeyProvider {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (!mod.equals(LibMisc.MOD_ID)) par3List.add(EnumChatFormatting.ITALIC + "[" + mod + "]");
    }
    ...
    public Achievement getAchievementOnPickup(ItemStack stack, EntityPlayer player, EntityItem item) {
        String type = getType(stack);
        if (type.equals(LibBlockNames.SUBTILE_DAYBLOOM)) return ModAchievements.daybloomPickup;
        else if (type.equals(LibBlockNames.SUBTILE_ENDOFLAME)) return ModAchievements.endoflamePickup;
        else if (type.equals(LibBlockNames.SUBTILE_KEKIMURUS)) return ModAchievements.kekimurusPickup;
        else if (type.equals(LibBlockNames.SUBTILE_HEISEI_DREAM)) return ModAchievements.heiseiDreamPickup;
        else if (type.equals(LibBlockNames.SUBTILE_POLLIDISIAC)) return ModAchievements.pollidisiacPickup;
        else if (type.equals(LibBlockNames.SUBTILE_BUBBELL)) return ModAchievements.bubbellPickup;
        else if (type.equals(LibBlockNames.SUBTILE_DANDELIFEON)) return ModAchievements.dandelifeonPickup;
        else if (type.equals("")) return ModAchievements.nullFlower;
        return null;
    }
}
