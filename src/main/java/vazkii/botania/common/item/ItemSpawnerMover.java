...
public class ItemSpawnerMover extends ItemMod {
    ...
    public static NBTTagCompound getSpawnerTag(ItemStack stack) {
        ...
        if (tag != null) {
            if (tag.hasKey(TAG_SPAWNER)) return tag.getCompoundTag(TAG_SPAWNER);
            if (tag.hasKey("EntityId")) return tag;
        }
    ...
    private static int getDelay(ItemStack stack) {
        NBTTagCompound tag = stack.getTagCompound();
        if (tag != null) return tag.getInteger(TAG_PLACE_DELAY);
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List infoList, boolean advancedTooltips) {
        ...
        if (id != null) infoList.add(StatCollector.translateToLocal("entity." + id + ".name"));
    }
    ...
    public boolean onItemUse(
        ...
        if (getEntityId(itemstack) == null) {
            ...
            if (getDelay(itemstack) <= 0
                    && placeBlock(itemstack, player, world, x, y, z, side, xOffset, yOffset, zOffset)) return true;
            return false;
        }
    }
    ...
    private boolean placeBlockAt(
            ...
            float hitZ,
            int metadata) {
        if (!world.setBlock(x, y, z, Blocks.mob_spawner, metadata, 3)) return false;

        Block block = world.getBlock(x, y, z);
        if (block.equals(Blocks.mob_spawner)) {
            TileEntity te = world.getTileEntity(x, y, z);
            NBTTagCompound tag = stack.getTagCompound();
            if (tag.hasKey(TAG_SPAWNER)) tag = tag.getCompoundTag(TAG_SPAWNER);
            tag.setInteger("x", x);
            tag.setInteger("y", y);
            tag.setInteger("z", z);
            te.readFromNBT(tag);
