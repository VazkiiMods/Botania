...
public class ExoflameFurnaceHandler {
	...
	public static boolean canSmelt(AbstractFurnaceTileEntity furnace, IRecipe<?> recipe) {
		return ((AccessorAbstractFurnaceTileEntity) furnace).botania_canSmelt(recipe);
	}
	...
	private static class FurnaceExoflameHeatable implements IExoflameHeatable {
		...
		public void boostCookTime() {
			int cookTime = ((AccessorAbstractFurnaceTileEntity) furnace).getCurrentCookTime();
			int cookTimeTotal = ((AccessorAbstractFurnaceTileEntity) furnace).getCookTimeTotal();
			((AccessorAbstractFurnaceTileEntity) furnace).setCookTime(Math.min(cookTimeTotal - 1, cookTime + 1));
		}
	}
}
