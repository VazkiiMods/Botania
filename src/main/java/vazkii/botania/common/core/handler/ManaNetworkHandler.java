...
public final class ManaNetworkHandler implements IManaNetwork {
    ...
    public TileEntity getClosestPool(ChunkCoordinates pos, World world, int limit) {
        if (manaPools.containsKey(world)) return getClosest(manaPools.get(world), pos, world.isRemote, limit);
        return null;
    }

    @Override
    public TileEntity getClosestCollector(ChunkCoordinates pos, World world, int limit) {
        if (manaCollectors.containsKey(world)) return getClosest(manaCollectors.get(world), pos, world.isRemote, limit);
        return null;
    }
    ...
    private synchronized boolean isIn(TileEntity tile, Map<World, List<TileSignature>> map) {
        ...
        for (TileSignature sig : list) if (sig.tile == tile) return true;

        return false;
    }

    private synchronized TileEntity getClosest(
            List<TileSignature> tiles, ChunkCoordinates pos, boolean remoteCheck, int limit) {
        float closest = Float.MAX_VALUE;
        TileEntity closestTile = null;

        for (TileSignature sig : tiles) {
            if (sig.remoteWorld != remoteCheck) continue;

            TileEntity tile = sig.tile;
            if (tile.isInvalid()) continue;
            float distance =
                    MathHelper.pointDistanceSpace(tile.xCoord, tile.yCoord, tile.zCoord, pos.posX, pos.posY, pos.posZ);
    ...
    private synchronized void remove(Map<World, List<TileSignature>> map, TileEntity tile) {
        ...
        if (!map.containsKey(world)) return;
    ...
    private synchronized void add(Map<World, List<TileSignature>> map, TileEntity tile) {
        ...
        if (!map.containsKey(world)) map.put(world, new ArrayList());

        tiles = map.get(world);

        if (!tiles.contains(tile)) tiles.add(new TileSignature(tile, tile.getWorldObj().isRemote));
    }
    ...
    private List<TileSignature> getAllInWorld(Map<World, List<TileSignature>> map, World world) {
        if (!map.containsKey(world)) return new ArrayList();
