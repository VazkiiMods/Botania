    ...
    public void onWorldDecoration(DecorateBiomeEvent.Decorate event) {
        if ((event.getResult() == Result.ALLOW || event.getResult() == Result.DEFAULT)
            ...
            if (event.world.provider instanceof IFlowerlessWorld)
                ...
                flowers = ((IFlowerlessBiome) event.world.getBiomeGenForCoords(event.chunkX, event.chunkZ))
                        .canGenerateFlowers(event.world, event.chunkX, event.chunkZ);

            if (!flowers) return;

            if (ConfigHandler.flowerDimensionWhitelist.length == 0
                    || Arrays.stream(ConfigHandler.flowerDimensionWhitelist)
                            .anyMatch(d -> d == event.world.provider.dimensionId)) {
                if (Arrays.stream(ConfigHandler.flowerDimensionBlacklist)
                        .noneMatch(d -> d == event.world.provider.dimensionId)) {
                    generateFlowers(event);
                }
            }

            if (ConfigHandler.mushroomDimensionWhitelist.length == 0
                    || Arrays.stream(ConfigHandler.mushroomDimensionWhitelist)
                            .anyMatch(d -> d == event.world.provider.dimensionId)) {
                if (Arrays.stream(ConfigHandler.mushroomDimensionBlacklist)
                        .noneMatch(d -> d == event.world.provider.dimensionId)) {
            ...
            if (event.rand.nextInt(ConfigHandler.flowerPatchChance) == 0) {
                ...
                for (int j = 0; j < ConfigHandler.flowerDensity * ConfigHandler.flowerPatchChance; j++) {
                    ...
                    if (event.world.isAirBlock(x1, y1, z1)
                            && (!event.world.provider.hasNoSky || y1 < 127)
                            && ModBlocks.flower.canBlockStay(event.world, x1, y1, z1)) {
                        if (primus) {
                            ...
                            flower.setSubTile(
                                    event.rand.nextBoolean()
                                            ? LibBlockNames.SUBTILE_NIGHTSHADE_PRIME
                                            : LibBlockNames.SUBTILE_DAYBLOOM_PRIME);
                            ...
                            if (event.rand.nextDouble() < ConfigHandler.flowerTallChance
                                    && ((BlockModFlower) ModBlocks.flower)
                                            .func_149851_a(event.world, x1, y1, z1, false))
