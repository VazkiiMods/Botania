...
public final class TerrasteelCraftingHandler {
    ...
    public static void onEntityUpdate(EntityItem item) {
        ...
        if (stack != null && stack.getItem() == ModItems.manaResource && stack.getItemDamage() == 0) {
            ...
            if (time != -1) {
                ...
                if (time == TIME) item.worldObj.playSoundAtEntity(item, "botania:terrasteelCraft", 1F, 1F);
    ...
    static int validateCraftingItem(EntityItem item) {
        ...
        if (estack.stackSize != 1) return -1;
        ...
        List<EntityItem> items = item.worldObj.getEntitiesWithinAABB(
                EntityItem.class, AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1));
    ...
    static void doParticles(EntityItem item, int ticks) {
        if (item.worldObj.isRemote) {
            ...
            for (int i = 0; i < totalSpiritCount; i++) {
                ...
                float[] colorsfx = new float[] {0F, (float) ticks / (float) TIME, 1F - (float) ticks / (float) TIME};
                Botania.proxy.wispFX(
                        item.worldObj, x, y, z, colorsfx[0], colorsfx[1], colorsfx[2], 0.85F, (float) g * 0.05F, 0.25F);
                Botania.proxy.wispFX(
                        item.worldObj,
                        x,
                        y,
