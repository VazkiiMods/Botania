...
public class InternalMethodHandler extends DummyMethodHandler {
    ...
    public IIcon getSubTileIconForName(String name) {
        IIcon icon =
                (ConfigHandler.altFlowerTextures ? BlockSpecialFlower.iconsAlt : BlockSpecialFlower.icons).get(name);
    ...
    @Override
    public void drawComplexManaHUD(
            int color,
            int mana,
            int maxMana,
            String name,
    ...
    public ItemStack getBindDisplayForFlowerType(SubTileEntity e) {
        return e instanceof SubTileGenerating
                ? new ItemStack(ModBlocks.spreader)
                : e instanceof SubTileFunctional ? new ItemStack(ModBlocks.pool) : new ItemStack(ModItems.twigWand);
    }
