...
public final class SheddingHandler {
    ...
    public static ShedPattern getShedPattern(Entity entity) {
        for (ShedPattern pattern : patterns) if (pattern.EntityClass.isInstance(entity)) return pattern;
    ...
    public static void addToLexicon() {
        if (!hasShedding()) return;

        int i = 1;
        for (ShedPattern pattern : patterns) {
            PageShedding page = new PageShedding(
                    String.valueOf(i),
                    (String) EntityList.classToStringMapping.get(pattern.EntityClass),
                    pattern.lexiconSize,
                    pattern.getItemStack());
    ...
    public static void loadFromConfig(Configuration config) {
        ...
        for (Object o : EntityList.stringToClassMapping.entrySet()) {
            ...
            if (EntityLiving.class.isAssignableFrom(entry.getValue())) {
                ...
                if (!defaultNames.contains(name)) loadFromConfig(config, name, null);
            }
        }
    }
    ...
    public static void loadFromConfig(Configuration config, String key, ShedPattern defaultPattern) {
        ...
        if (defaultPattern != null) {
            itemName = Item.itemRegistry.getNameForObject(
                    defaultPattern.getItemStack().getItem());
        ...
        lexiconSize =
                config.get("Shedding", key + ".lexiconDisplaySize", lexiconSize).getInt();

        if (itemName != null && !itemName.isEmpty() && rate != -1)
            patterns.add(new ShedPattern(
                    (Class<?>) EntityList.stringToClassMapping.get(key),
                    new ItemStack((Item) Item.itemRegistry.getObject(itemName), 1, metadata),
                    rate,
                    lexiconSize));
    }
