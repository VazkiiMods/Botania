...
public class PixieHandler {
    ...
    public void onDamageTaken(LivingHurtEvent event) {
        if (!event.entityLiving.worldObj.isRemote
            ...
            for (ItemStack element : player.inventory.armorInventory) chance += getChance(element);

            InventoryBaubles baubles = PlayerHandler.getPlayerBaubles(player);
            for (int i = 0; i < baubles.getSizeInventory(); i++) chance += getChance(baubles.getStackInSlot(i));
            ...
            if (Math.random() < chance) {
                ...
                if (((ItemElementiumHelm) ModItems.elementiumHelm).hasArmorSet(player)) {
                    int[] potions =
                            new int[] {Potion.blindness.id, Potion.wither.id, Potion.moveSlowdown.id, Potion.weakness.id
                            };
                    pixie.setApplyPotionEffect(
                            new PotionEffect(potions[event.entity.worldObj.rand.nextInt(potions.length)], 40, 0));
                }

                float dmg = 4;
                if (stack != null && stack.getItem() == ModItems.elementiumSword) dmg += 2;
    ...
    float getChance(ItemStack stack) {
        ...
        if (item instanceof IPixieSpawner) return ((IPixieSpawner) item).getPixieChance(stack);
