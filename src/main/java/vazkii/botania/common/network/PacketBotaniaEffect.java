			...
			ctx.get().enqueueWork(new Runnable() {
				...
				public void run() {
					...
					case PAINT_LENS: {
						...
						int b = hex & 0xFF;
						for (int i = 0; i < 10; i++) {
							BlockPos pos = new BlockPos(message.x, message.y, message.z).offset(Direction.getRandomDirection(world.rand));
					...
					case ARENA_INDICATOR: {
						...
						for (int i = 0; i < 360; i += 8) {
							...
							Botania.proxy.addParticleForceNear(world, data, x, y, z, 0, 0, 0);
						}
						break;
					}
					case ITEM_SMOKE: {
					...
					case TERRA_PLATE: {
						...
						if (te instanceof TileTerraPlate) {
							...
							for (int i = 0; i < totalSpiritCount; i++) {
								...
								Botania.proxy.addParticleForceNear(world, data, x, y, z, 0, (float) (-g * 0.05), 0);
