...
public class CosmeticAttachRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting var1, World var2) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                if (stack.getItem() instanceof ICosmeticBauble && !foundCosmetic) foundCosmetic = true;
                else if (!foundAttachable) {
                    if (stack.getItem() instanceof ICosmeticAttachable
                            && !(stack.getItem() instanceof ICosmeticBauble)
                            && ((ICosmeticAttachable) stack.getItem()).getCosmeticItem(stack) == null)
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null) {
                if (stack.getItem() instanceof ICosmeticBauble && cosmeticItem == null) cosmeticItem = stack;
                else attachableItem = stack;
            }
        }

        ICosmeticAttachable attachable = (ICosmeticAttachable) attachableItem.getItem();
        if (attachable.getCosmeticItem(attachableItem) != null) return null;
