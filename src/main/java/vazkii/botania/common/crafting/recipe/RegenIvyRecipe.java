...
public class RegenIvyRecipe implements IRecipe {
            ...
            if (stack != null) {
                ...
                if (item.isRepairable()
                        && !(ItemNBTHelper.detectNBT(stack)
                                && ItemNBTHelper.getBoolean(stack, ItemRegenIvy.TAG_REGEN, false))) tool = stack;
                else if (item == ModItems.regenIvy) foundIvy = true;
            }
        }
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null) {
                ...
                if (tool != null && tool.getItem().getIsRepairable(tool, stack)) materialsFound++;
                else if (stack != tool && item != ModItems.regenIvy) return false;
            }
        }
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null && stack.getItem().isDamageable()) tool = stack;
        }
