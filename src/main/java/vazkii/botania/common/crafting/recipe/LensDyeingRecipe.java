...
public class LensDyeingRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting var1, World var2) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                if (stack.getItem() instanceof ILens && !foundLens) foundLens = true;
                else if (!foundDye) {
                    int color = getStackColor(stack);
                    if (color > -1) foundDye = true;
                    else return false;
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null) {
                if (stack.getItem() instanceof ILens && lens == null) lens = stack;
                else
                    color = getStackColor(
                            stack); // We can assume if its not a lens its a dye because we checked it in matches()
            }
        }
