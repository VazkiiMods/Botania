...
public class AncientWillRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting var1, World var2) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                if (stack.getItem() == ModItems.ancientWill && !foundWill) foundWill = true;
                else if (!foundItem) {
                    if (stack.getItem() instanceof IAncientWillContainer) foundItem = true;
                    else return false;
                }
            }
        }
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null) {
                if (stack.getItem() instanceof IAncientWillContainer && item == null) item = stack;
                else will = stack.getItemDamage();
            }
        }

        IAncientWillContainer container = (IAncientWillContainer) item.getItem();
        if (container.hasAncientWill(item, will)) return null;
