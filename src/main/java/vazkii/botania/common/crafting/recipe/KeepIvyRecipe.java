...
public class KeepIvyRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting var1, World var2) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                if (stack.getItem() == ModItems.keepIvy) foundIvy = true;
                else if (!foundItem
                        && !(ItemNBTHelper.detectNBT(stack)
                                && ItemNBTHelper.getBoolean(stack, ItemKeepIvy.TAG_KEEP, false))
                        && !stack.getItem().hasContainerItem(stack)) foundItem = true;
                else return false;
            }
        }
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null && stack.getItem() != ModItems.keepIvy) item = stack;
        }
