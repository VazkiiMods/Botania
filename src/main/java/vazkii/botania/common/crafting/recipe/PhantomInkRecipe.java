...
public class PhantomInkRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting var1, World var2) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                if (stack.getItem() == ModItems.phantomInk && !foundInk) foundInk = true;
                else if (!foundItem) {
                    if (stack.getItem() instanceof IPhantomInkable
                            && stack.getItem().getContainerItem(stack) == null) foundItem = true;
                    else return false;
                } else return false;
            }
        }
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1) {
        ...
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ItemStack stack = var1.getStackInSlot(i);
            if (stack != null && stack.getItem() instanceof IPhantomInkable && item == null) item = stack;
        }
