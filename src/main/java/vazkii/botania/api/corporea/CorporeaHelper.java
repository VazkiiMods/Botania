...
public final class CorporeaHelper {
    ...
    private static final List<ICorporeaAutoCompleteController> autoCompleteControllers =
            new ArrayList<ICorporeaAutoCompleteController>();
     ...
     * called instead if the context for this exists to avoid having to get the value again.
     */
    public static Map<IInventory, Integer> getInventoriesWithItemInNetwork(
            ItemStack stack, ICorporeaSpark spark, boolean checkNBT) {
        List<IInventory> inventories = getInventoriesOnNetwork(spark);
     ...
     */
    public static Map<IInventory, Integer> getInventoriesWithItemInNetwork(
            ItemStack stack, List<IInventory> inventories, boolean checkNBT) {
     ...
     */
    public static List<ItemStack> requestItem(
            Object matcher, int itemCount, ICorporeaSpark spark, boolean checkNBT, boolean doit) {
        ...
        for (ICorporeaInterceptor interceptor : interceptors.keySet())
            interceptor.interceptRequestLast(
                    matcher, itemCount, interceptors.get(interceptor), spark, stacks, inventories, doit);
    ...
    public static ICorporeaSpark getSparkForBlock(World world, int x, int y, int z) {
        List<ICorporeaSpark> sparks = world.getEntitiesWithinAABB(
                ICorporeaSpark.class, AxisAlignedBB.getBoundingBox(x, y + 1, z, x + 1, y + 2, z + 1));
    ...
    public static boolean stacksMatch(ItemStack stack, String s) {
        ...
        return equalOrContain(name, s, contains)
                || equalOrContain(name + "s", s, contains)
