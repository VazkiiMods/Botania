...
public class StonecuttingProvider extends RecipeProvider {
	...
	private static void registerForPavement(String color, Consumer<IFinishedRecipe> consumer) {
		Block base = Registry.BLOCK.func_241873_b(prefix(color + LibBlockNames.PAVEMENT_SUFFIX)).get();
		Block slab = Registry.BLOCK.func_241873_b(prefix(color + LibBlockNames.PAVEMENT_SUFFIX + LibBlockNames.SLAB_SUFFIX)).get();
		Block stair = Registry.BLOCK.func_241873_b(prefix(color + LibBlockNames.PAVEMENT_SUFFIX + LibBlockNames.STAIR_SUFFIX)).get();
		consumer.accept(stonecutting(base, slab, 2));
