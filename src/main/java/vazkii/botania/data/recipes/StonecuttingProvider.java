...
public class StonecuttingProvider extends RecipeProvider {
	...
	private void registerForQuartz(String variant, Consumer<IFinishedRecipe> consumer) {
	...
	private void registerForPavement(String color, Consumer<IFinishedRecipe> consumer) {
	...
	private void registerForMetamorphic(String variant, Consumer<IFinishedRecipe> consumer) {
	...
	protected ResourceLocation idFor(IItemProvider a, IItemProvider b) {
	...
	protected IFinishedRecipe stonecutting(IItemProvider input, IItemProvider output) {
		return stonecutting(input, output, 1);
	}

	protected IFinishedRecipe stonecutting(IItemProvider input, IItemProvider output, int count) {
		return new Result(idFor(input, output), IRecipeSerializer.STONECUTTING, Ingredient.fromItems(input), output.asItem(), count);
	}

	protected IFinishedRecipe anyToAnyStonecutting(List<? extends IItemProvider> inputs, IItemProvider output) {
