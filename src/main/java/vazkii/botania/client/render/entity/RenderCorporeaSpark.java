...
public class RenderCorporeaSpark extends RenderSparkBase<EntityCorporeaSpark> {
    ...
    public void colorSpinningIcon(EntityCorporeaSpark entity, float a) {
        ...
        GL11.glColor4f(
                EntitySheep.fleeceColorTable[network][0],
                EntitySheep.fleeceColorTable[network][1],
                EntitySheep.fleeceColorTable[network][2],
                a);
    }
    ...
    public void renderCallback(EntityCorporeaSpark entity, float pticks) {
        ...
        Minecraft.getMinecraft()
                .renderEngine
                .bindTexture(block ? TextureMap.locationBlocksTexture : TextureMap.locationItemsTexture);
        IIcon icon = block
                ? Block.getBlockFromItem(item).getBlockTextureFromSide(ForgeDirection.UP.ordinal())
                : item.getIcon(stack, 0);
            ...
            for (int i = -shift; i < shift; i++) {
                ...
                for (int j = -shift; j < shift; j++) {
                    ...
                    ItemRenderer.renderItemIn2D(
                            ...
                            minU + stepU * (i + shift),
                            minV + stepV * (j + shift + 1),
                            minU + stepU * (i + shift + 1),
                            minV + stepV * (j + shift),
                            icon.getIconWidth() / pieces,
                            icon.getIconHeight() / pieces,
                            1F / 8F);
                    GL11.glTranslated(0F, -gap * j, 0F);
                }
                GL11.glTranslated(-gap * i, 0F, 0F);
