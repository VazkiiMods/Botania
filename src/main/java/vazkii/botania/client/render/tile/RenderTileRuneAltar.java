...
public class RenderTileRuneAltar extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float partticks) {
        ...
        for (int i = 0; i < altar.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                ...
                if (!ForgeHooksClient.renderEntityItem(
                        new EntityItem(altar.getWorldObj(), altar.xCoord, altar.yCoord, altar.zCoord, stack),
                        stack,
                        0F,
                        0F,
                        altar.getWorldObj().rand,
                        mc.renderEngine,
                        renderBlocks,
                        1)) {
                    GL11.glScalef(0.5F, 0.5F, 0.5F);
        ...
        float scale =
                altar.getTargetMana() == 0 ? 0 : (float) altar.getCurrentMana() / (float) altar.getTargetMana() / 75F;
