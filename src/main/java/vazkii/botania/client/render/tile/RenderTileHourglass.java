...
public class RenderTileHourglass extends BlockEntityRenderer<TileHourglass> {
	...
	final Identifier texture = new Identifier(LibResources.MODEL_HOURGLASS);
	...
	@Override
	public void render(@Nullable TileHourglass hourglass, float ticks, MatrixStack ms, VertexConsumerProvider buffers, int light, int overlay) {
		ms.push();
		...
		ItemStack stack = hasWorld ? hourglass.getItemHandler().getStack(0) : ItemStack.EMPTY;
