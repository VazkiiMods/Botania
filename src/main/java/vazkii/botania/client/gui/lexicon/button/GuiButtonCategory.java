...
public class GuiButtonCategory extends GuiButtonLexicon {
    ...
    public void drawButton(Minecraft mc, int mx, int my) {
        ...
        if (inside) ticksHovered = Math.min(time, ticksHovered + gui.timeDelta);
        else ticksHovered = Math.max(0F, ticksHovered - gui.timeDelta);

        if (resource == null) {
            if (category == null) resource = fallbackResource;
            else resource = category.getIcon();
            if (resource == null) resource = fallbackResource;
        }
        ...
        if (inside) gui.categoryHighlight = StatCollector.translateToLocal(getTooltipText());
    }

    String getTooltipText() {
        if (category == null) return "botaniamisc.lexiconIndex";
