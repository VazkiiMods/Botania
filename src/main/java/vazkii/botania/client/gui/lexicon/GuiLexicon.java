...
public class GuiLexicon extends GuiScreen {
    ...
    public final void initGui() {
        ...
        if (notes.containsKey(key)) note = notes.get(key);
    ...
    public void updateScreen() {
        if (notesEnabled && notesMoveTime < NOTE_TWEEN_TIME) notesMoveTime++;
        else if (!notesEnabled && notesMoveTime > 0) notesMoveTime--;
    }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (ClientProxy.jingleTheBells) drawTexturedModalRect(left + 3, top + 1, 0, 212, 138, 6);

        String subtitle = getSubtitle();
        if (subtitle != null) drawBookmark(left + guiWidth / 2, top - getTitleHeight() + 10, subtitle, true, 191);
        drawBookmark(left + guiWidth / 2, top - getTitleHeight(), getTitle(), true);

        if (isMainPage()) drawHeader();
        ...
        if (mc.thePlayer.getCommandSenderName().equals("haighyorkie")) {
            ...
            if (par1 >= left - 19 && par1 < left && par2 >= top + 62 && par2 < top + 88) {
                ...
                int y = (int)
                        (top + guiHeight / 2 - h / 4 + Math.sin((ClientTickHandler.ticksInGame + par3) / 6.0) * 40);
                ...
                RenderHelper.renderTooltip(
                        ...
                        par2,
                        Arrays.asList(
                                EnumChatFormatting.GOLD + "#goldfishchris",
                                EnumChatFormatting.AQUA + "IT SAYS MANUAL"));
            }
        }
        ...
        if (hasTutorialArrow) {
            ...
            GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
            GL11.glColor4f(
                    1F, 1F, 1F, 0.7F + (float) (Math.sin((ClientTickHandler.ticksInGame + par3) * 0.3F) + 1) * 0.15F);
    ...
    public void drawNotes(float part) {
        ...
        if (notesMoveTime < NOTE_TWEEN_TIME && notesEnabled) time += part;
        else if (notesMoveTime > 0 && !notesEnabled) time -= part;
        ...
        if (notesEnabled && ClientTickHandler.ticksInGame % 20 < 10) noteDisplay += "&r_";
    ...
    public void drawBookmark(int x, int y, String s, boolean drawLeft, int color, int v) {
        ...
        if (drawLeft) drawTexturedModalRect(x - l / 2 - 9, y - 1, 61, v, 6, 11);
        for (int i = 0; i < l + 6; i++) drawTexturedModalRect(x - l / 2 - 3 + i, y - 1, 60, v, 1, 11);
    ...
    void drawHeader() {
        ...
        fontRendererObj.drawString(
                String.format(StatCollector.translateToLocal("botaniamisc.edition"), ItemLexicon.getEdition()),
                left + 24,
                top + 22,
                color);
                    ...
                    if (isShiftKeyDown()) {
                        try {
                            if (Desktop.isDesktopSupported())
                                Desktop.getDesktop()
                                        .browse(new URI("http://botaniamod.net/changelog.php#"
                                                + PersistentVariableHelper.lastBotaniaVersion.replaceAll(
                                                        "\\.|\\s", "-")));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    public void handleBookmark(GuiButton par1GuiButton) {
        ...
        if (i == bookmarks.size()) {
            ...
            if (isShiftKeyDown()) {
                ...
                if (!bookmark.getTitle().equals(getTitle())) {
                    ...
                    if (bookmark instanceof IParented) ((IParented) bookmark).setParent(this);
                    ClientTickHandler.notifyPageChange();
                }
            }
        }
        ...
        if (modified) PersistentVariableHelper.saveSafe();
    }
    ...
    public int bookmarkWidth(String b) {
        if (fontRendererObj == null) fontRendererObj = Minecraft.getMinecraft().fontRenderer;
    ...
    void populateIndex() {
        ...
        for (int i = shift; i < 12; i++) {
            ...
            if (category != null) button.displayString = StatCollector.translateToLocal(category.getUnlocalizedName());
            else {
    ...
    void populateBookmarks() {
        ...
        for (GuiButton button : buttons) if (button.id >= BOOKMARK_START) remove.add(button);
        buttonList.removeAll(remove);

        int len = bookmarks.size();
        boolean thisExists = false;
        for (GuiLexicon lex : bookmarks) if (lex.getTitle().equals(getTitle())) thisExists = true;
        ...
        for (int i = 0; i < len + (addEnabled ? 1 : 0); i++) {
            ...
            buttonList.add(new GuiButtonBookmark(
                    BOOKMARK_START + i,
                    left + 138,
                    top + 18 + 14 * i,
                    gui == null ? this : gui,
        ...
        if (isMainPage())
            buttonList.add(new GuiButtonHistory(
                    BOOKMARK_START + MAX_BOOKMARK_COUNT,
                    left + 138,
                    top + guiHeight - 24,
                    StatCollector.translateToLocal("botaniamisc.history"),
                    this));
        else if (isChallenge())
            buttonList.add(new GuiButtonChallengeInfo(
                    BOOKMARK_START + MAX_BOOKMARK_COUNT,
                    left + 138,
                    top + guiHeight - 24,
                    StatCollector.translateToLocal("botaniamisc.info"),
                    this));
    }
    ...
    public static void startTutorial() {
        ...
        if (ConfigHandler.hardcorePassiveGeneration > 0) tutorial.add(LexiconData.generatingIntro);
    ...
    public final void putTutorialArrow() {
        ...
        if (hasTutorialArrow) positionTutorialArrow();
    }
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        if (par2 == KONAMI_CODE[konamiIndex]) {
            ...
            if (konamiIndex >= KONAMI_CODE.length) {
                mc.getSoundHandler()
                        .playSound(PositionedSoundRecord.func_147673_a(new ResourceLocation("botania:way")));
                konamiIndex = 0;
            }
        } else konamiIndex = 0;
    ...
    public void handleNoteKey(char par1, int par2) {
        if (notesEnabled) {
            ...
            if (par2 == 14 && note.length() > 0) {
                if (isCtrlKeyDown()) note = "";
                else {
                    if (note.endsWith("<br>")) note = note.substring(0, note.length() - 4);
    ...
    public static GuiLexicon create(NBTTagCompound cmp) {
        ...
        try {
            ...
            if (lex != null) lex.load(cmp);
            if (isValidLexiconGui(lex)) return lex;
            return null;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static boolean isValidLexiconGui(GuiLexicon gui) {
        ...
        if (gui.isCategoryIndex() || gui.isChallenge()) return true;
        if (gui.isIndex()) {
            GuiLexiconIndex indexGui = (GuiLexiconIndex) gui;
            if (indexGui.category == null) return true;
            return BotaniaAPI.getAllCategories().contains(indexGui.category);
        }

        GuiLexiconEntry entryGui = (GuiLexiconEntry) gui;
        if (!BotaniaAPI.getAllEntries().contains(entryGui.entry)) return false;
