...
public class GuiLexiconChallenge extends GuiLexicon implements IParented {
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        RenderItem.getInstance()
                .renderItemIntoGUI(fontRendererObj, mc.renderEngine, challenge.icon, left + 18, top + 15);
        ...
        fontRendererObj.drawString(
                EnumChatFormatting.BOLD + StatCollector.translateToLocal(challenge.unlocalizedName),
                left + 38,
                top + 13,
                0);
        fontRendererObj.drawString(
                StatCollector.translateToLocal(challenge.level.getName()) + " / "
                        + (challenge.complete ? EnumChatFormatting.DARK_GREEN : EnumChatFormatting.DARK_RED)
                        + StatCollector.translateToLocal(
                                challenge.complete ? "botaniamisc.completed" : "botaniamisc.notCompleted"),
                left + 38,
                top + 23,
                0);
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        if (par1GuiButton.id >= BOOKMARK_START) super.actionPerformed(par1GuiButton);
    ...
    void setCompleteButtonTitle() {
        completeButton.displayString = StatCollector.translateToLocal(
                challenge.complete ? "botaniamisc.markNotCompleted" : "botaniamisc.markCompleted");
    }
