...
public class GuiLexiconIndex extends GuiLexicon implements IParented {
    ...
    public void setTitle() {
        title = StatCollector.translateToLocal(
                category == null ? "botaniamisc.lexiconIndex" : category.getUnlocalizedName());
    }
    ...
    void buildEntries() {
        ...
        for (LexiconEntry entry : category == null ? BotaniaAPI.getAllEntries() : category.entries) {
            if (entry.isVisible()
                    && lex.isKnowledgeUnlocked(stackUsed, entry.getKnowledgeType())
                    && matchesSearch(entry)) entriesToDisplay.add(entry);
        }
    ...
    boolean matchesSearch(LexiconEntry e) {
        ...
        if (StatCollector.translateToLocal(e.getUnlocalizedName()).toLowerCase().contains(search)) return true;

        for (ItemStack stack : e.getDisplayedRecipes()) {
            String stackName = stack.getDisplayName().toLowerCase().trim();
            if (stackName.contains(search)) return true;
        }
        ...
        for (int i = page * 12; i < (page + 1) * 12; i++) {
            ...
            if (entry != null) {
                button.displayString =
                        entry.getKnowledgeType().color + "" + (entry.isPriority() ? EnumChatFormatting.ITALIC : "")
                                + StatCollector.translateToLocal(entry.getUnlocalizedName());
                button.displayStack = entry.getIcon();
                if (entry == tutEntry) tutPage = page;

                if (entry instanceof DLexiconEntry) button.dog = true;
        ...
        if (!searchField.getText().isEmpty()) {
            ...
            if (entriesToDisplay.size() == 1) {
                ...
                mc.fontRenderer.drawString(
                        s, left + guiWidth / 2 - mc.fontRenderer.getStringWidth(s) / 2, top + 30, 0x666666);
            ...
            mc.fontRenderer.drawString(
                    s, left + 120 - mc.fontRenderer.getStringWidth(s), top + guiHeight - 18, 0x666666);
            mc.fontRenderer.setUnicodeFlag(unicode);
        }

        float animationTime = 4F;
        if (isShiftKeyDown()) {
            if (currentButton != null) infoTime = Math.min(animationTime, infoTime + timeDelta);
        } else {
            ...
            if (currentButton != null && infoTime == 0) {
                ...
                mc.fontRenderer.drawStringWithShadow(
                        EnumChatFormatting.BOLD + "Shift", x * 2 - 6, y * 2 + 20, 0xFFFFFF);
                ...
                for (int i = 0; i < stacks.size(); i++) {
                    ...
                    RenderItem.getInstance()
                            .renderItemAndEffectIntoGUI(
                                    mc.fontRenderer, mc.renderEngine, stack, 38 + spaceForEach * i, -28);
                }
            }
    ...
    public void positionTutorialArrow() {
        ...
        for (GuiButton button : buttons) {
            ...
            if (index >= entriesToDisplay.size()) continue;
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        if (par1GuiButton.id >= BOOKMARK_START) handleBookmark(par1GuiButton);
        else if (par1GuiButton.id == NOTES_BUTTON_ID) notesEnabled = !notesEnabled;
        else
    ...
    public void handleMouseInput() {
        ...
        if (Mouse.getEventButton() == 0) swiped = false;

        int w = Mouse.getEventDWheel();
        if (w < 0) nextPage();
        else if (w > 0) prevPage();
    }
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        if (!notesEnabled) {
            ...
            if (!searchField.getText().equalsIgnoreCase(search)) updateAll();
        }
    ...
    public void load(NBTTagCompound cmp) {
        ...
        if (categoryStr.isEmpty()) category = null;
        else
