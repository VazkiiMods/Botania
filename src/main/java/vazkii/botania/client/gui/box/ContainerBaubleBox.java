...
public class ContainerBaubleBox extends Container {
    ...
    public ContainerBaubleBox(EntityPlayer player) {
        ...
        if (!player.worldObj.isRemote) baubles.stackList = PlayerHandler.getPlayerBaubles(player).stackList;
        ...
        for (i = 0; i < 3; ++i)
            for (j = 0; j < 9; ++j) addSlotToContainer(new Slot(playerInv, j + i * 9 + 9, 8 + j * 18, 84 + i * 18));

        for (i = 0; i < 9; ++i) {
            if (player.inventory.currentItem == i) addSlotToContainer(new SlotLocked(playerInv, i, 8 + i * 18, 142));
    ...
    public ItemStack transferStackInSlot(EntityPlayer p_82846_1_, int p_82846_2_) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (p_82846_2_ < 28) {
                if (!mergeItemStack(itemstack1, 28, 64, true)) return null;
            } else {
                if (itemstack1 != null
                        && (itemstack1.getItem() instanceof IBauble || itemstack1.getItem() instanceof IManaItem)
                        && !mergeItemStack(itemstack1, 4, 28, false)) return null;
            }
