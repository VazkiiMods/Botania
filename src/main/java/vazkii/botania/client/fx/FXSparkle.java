...
public class FXSparkle extends EntityFX {
    ...
    public static void dispatchQueuedRenders(Tessellator tessellator) {
        ...
        for (FXSparkle sparkle : queuedRenders) sparkle.renderQueued(tessellator);
        tessellator.draw();

        ShaderHelper.useShader(ShaderHelper.filmGrain);
        tessellator.startDrawingQuads();
        for (FXSparkle sparkle : queuedCorruptRenders) sparkle.renderQueued(tessellator);
        tessellator.draw();
    ...
    private void renderQueued(Tessellator tessellator) {
        if (fake) ParticleRenderDispatcher.fakeSparkleFxCount++;
    ...
    public void renderParticle(Tessellator tessellator, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        if (corrupt) queuedCorruptRenders.add(this);
