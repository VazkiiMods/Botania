...
public class FXWisp extends EntityFX {
    ...
    public static void dispatchQueuedRenders(Tessellator tessellator) {
        ...
        Minecraft.getMinecraft()
                .renderEngine
                .bindTexture(ConfigHandler.matrixMode ? ObfuscationHelper.getParticleTexture() : particles);

        if (!queuedRenders.isEmpty()) {
            tessellator.startDrawingQuads();
            for (FXWisp wisp : queuedRenders) wisp.renderQueued(tessellator, true);
            tessellator.draw();
        }
        ...
        if (!queuedDepthIgnoringRenders.isEmpty()) {
            ...
            tessellator.startDrawingQuads();
            for (FXWisp wisp : queuedDepthIgnoringRenders) wisp.renderQueued(tessellator, false);
            tessellator.draw();
    ...
    private void renderQueued(Tessellator tessellator, boolean depthEnabled) {
        if (depthEnabled) ParticleRenderDispatcher.wispFxCount++;
    ...
    public void renderParticle(Tessellator tessellator, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        if (depthTest) queuedRenders.add(this);
