...
public class RecipeHandlerLexicaBotania extends TemplateRecipeHandler {
    ...
    public void drawBackground(int recipe) {
        ...
        s = type.color
                + StatCollector.translateToLocal(type.getUnlocalizedName()).replaceAll("\\&.", "");
        ...
        if (GuiScreen.isShiftKeyDown()
                ...
                && Minecraft.getMinecraft().gameSettings.advancedItemTooltips) s = "name: " + key;
        else if (quickInfo.equals(quickInfoLocal)) s = StatCollector.translateToLocal("botania.nei.lexicaNoInfo");
        else {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals("botania.lexica")) {
            for (LexiconEntry entry : BotaniaAPI.getAllEntries()) {
                ...
                for (ItemStack stack : stacks) arecipes.add(new CachedLexicaBotaniaRecipe(stack, entry));
            }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (LexiconEntry entry : BotaniaAPI.getAllEntries()) {
            ...
            for (ItemStack stack : stacks) {
                ...
                if (key1.equals(key2)) arecipes.add(new CachedLexicaBotaniaRecipe(stack, entry));
            }
        }
    }
