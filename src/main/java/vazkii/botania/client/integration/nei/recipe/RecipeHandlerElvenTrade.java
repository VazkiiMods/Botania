...
public class RecipeHandlerElvenTrade extends TemplateRecipeHandler {
    ...
    private List<RecipeElvenTrade> filteredElvenTradeRecipes() {
        return BotaniaAPI.elvenTradeRecipes.stream()
                .filter(recipe -> {
                    if (recipe == null) {
                        return false;
                    }
    ...
    private boolean stackSame(ItemStack stack, Object obj) {
        if (obj instanceof String) {
            return OreDictionary.getOres((String) obj).stream()
                    .anyMatch(s -> ItemNBTHelper.areStacksSameTypeCraftingWithNBT(stack, s));
        } else {
            return Arrays.stream(NEIServerUtils.extractRecipeItems(obj))
                    .anyMatch(s -> ItemNBTHelper.areStacksSameTypeCraftingWithNBT(stack, s));
        }
    }
}
