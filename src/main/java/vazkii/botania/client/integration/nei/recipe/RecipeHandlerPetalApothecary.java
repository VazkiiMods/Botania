...
public class RecipeHandlerPetalApothecary extends TemplateRecipeHandler {
    ...
    public class CachedPetalApothecaryRecipe extends CachedRecipe {
        ...
        public CachedPetalApothecaryRecipe(RecipePetals recipe, boolean addCenterItem) {
            ...
            if (addCenterItem) inputs.add(new PositionedStack(new ItemStack(ModBlocks.altar), 73, 55));
        }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals(getRecipeID())) {
            for (RecipePetals recipe : getRecipes())
                if (recipe.getOutput().getItem() != Items.skull) arecipes.add(getCachedRecipe(recipe));
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (RecipePetals recipe : getRecipes()) {
            ...
            if (recipe.getOutput().stackTagCompound != null
                            && ItemNBTHelper.areStacksSameTypeWithNBT(recipe.getOutput(), result)
                    || recipe.getOutput().stackTagCompound == null
                            && NEIServerUtils.areStacksSameTypeCrafting(recipe.getOutput(), result)
                            && recipe.getOutput().getItem() != Items.skull) arecipes.add(getCachedRecipe(recipe));
        }
    }
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (RecipePetals recipe : getRecipes()) {
            ...
            if (ItemNBTHelper.cachedRecipeContainsWithNBT(crecipe.inputs, ingredient)
                    && recipe.getOutput().getItem() != Items.skull) arecipes.add(crecipe);
        }
    }
}
