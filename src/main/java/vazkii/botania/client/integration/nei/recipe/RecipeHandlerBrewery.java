...
public class RecipeHandlerBrewery extends TemplateRecipeHandler {
    ...
    public class CachedBreweryRecipe extends CachedRecipe {
        ...
        public CachedBreweryRecipe(RecipeBrew recipe, ItemStack vial) {
            ...
            ItemStack toVial;
            if (vial == null) toVial = new ItemStack(ModItems.vial);
            else toVial = vial.copy();
            toVial.stackSize = 1;
        ...
        public void setIngredients(List<Object> inputs) {
            ...
            for (Object o : inputs) {
                ...
                else this.inputs.add(new PositionedStack(o, left + i * 18, 6));
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals("botania.brewery"))
            for (RecipeBrew recipe : BotaniaAPI.brewRecipes) arecipes.add(new CachedBreweryRecipe(recipe));
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (ingredient.getItem() instanceof IBrewContainer) {
            for (RecipeBrew recipe : BotaniaAPI.brewRecipes) {
                ...
                if (recipe.getOutput(ingredient) != null) arecipes.add(new CachedBreweryRecipe(recipe, ingredient));
            }
        } else
            for (RecipeBrew recipe : BotaniaAPI.brewRecipes) {
                if (recipe == null) continue;
                ...
                if (ItemNBTHelper.cachedRecipeContainsWithNBT(crecipe.inputs, ingredient)) arecipes.add(crecipe);
            }
    }
}
