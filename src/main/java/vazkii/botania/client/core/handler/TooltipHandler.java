...
public final class TooltipHandler {
    ...
    public void onTooltipEvent(ItemTooltipEvent event) {
        ...
        } else if (event.itemStack.getItem() == Item.getItemFromBlock(Blocks.mob_spawner)
                && event.entityPlayer.capabilities.isCreativeMode)
            event.toolTip.add(StatCollector.translateToLocal("botaniamisc.spawnerTip"));

        if (ItemNBTHelper.detectNBT(event.itemStack)
                && ItemNBTHelper.getBoolean(event.itemStack, ItemRegenIvy.TAG_REGEN, false))
            event.toolTip.add(StatCollector.translateToLocal("botaniamisc.hasIvy"));
        if (ItemNBTHelper.detectNBT(event.itemStack)
                && ItemNBTHelper.getBoolean(event.itemStack, ItemKeepIvy.TAG_KEEP, false))
