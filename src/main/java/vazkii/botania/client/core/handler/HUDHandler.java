...
public final class HUDHandler {
    ...
    public void onDrawScreenPost(RenderGameOverlayEvent.Post event) {
        ...
        if (event.type == ElementType.ALL) {
            ...
            if (pos != null) {
                ...
                if (equippedStack != null) {
                    if (pos != null && equippedStack.getItem() == ModItems.twigWand) {
                        ...
                        if (block instanceof IWandHUD) {
                            ...
                            ((IWandHUD) block)
                                    .renderHUD(mc, event.resolution, mc.theWorld, pos.blockX, pos.blockY, pos.blockZ);
                            profiler.endSection();
                        }
                ...
                if (tile != null && tile instanceof TileAltar) ((TileAltar) tile).renderHUD(mc, event.resolution);
            ...
            if (!TileCorporeaIndex.getInputHandler()
                            .getNearbyIndexes(mc.thePlayer)
                            .isEmpty()
            ...
            if (MultiblockRenderHandler.currentMultiblock != null && MultiblockRenderHandler.anchor == null) {
                ...
                mc.fontRenderer.drawStringWithShadow(
                        s,
                        event.resolution.getScaledWidth() / 2 - mc.fontRenderer.getStringWidth(s) / 2,
                        event.resolution.getScaledHeight() / 2 - 30,
                        0xFFFFFF);
                profiler.endSection();
            }
            ...
            if (anyRequest) renderManaInvBar(event.resolution, creative, totalMana, totalMaxMana);
    ...
    private void renderWandModeDisplay(ScaledResolution res) {
        ...
        int ticks = ReflectionHelper.getPrivateValue(
                GuiIngame.class, mc.ingameGUI, LibObfuscation.REMAINING_HIGHLIGHT_TICKS);
        ticks -= 15;
        if (ticks > 0) {
            int alpha = Math.min(255, (int) (ticks * 256.0F / 10.0F));
            int color = 0x00CC00 + (alpha << 24);
            String disp =
                    StatCollector.translateToLocal(ItemTwigWand.getModeString(mc.thePlayer.getCurrentEquippedItem()));
    ...
    private void renderManaInvBar(ScaledResolution res, boolean hasCreative, int totalMana, int totalMaxMana) {
        ...
        if (!hasCreative) {
            if (totalMaxMana == 0) width = 0;
            else width *= (double) totalMana / (double) totalMaxMana;
        }

        if (width == 0) {
            if (totalMana > 0) width = 1;
            else return;
        }

        Color color = new Color(Color.HSBtoRGB(
                0.55F, (float) Math.min(1F, Math.sin(System.currentTimeMillis() / 200D) * 0.5 + 1F), 1F));
        GL11.glColor4ub(
                (byte) color.getRed(), (byte) color.getGreen(), (byte) color.getBlue(), (byte) (255 - color.getRed()));
        ...
        for (RecipeManaInfusion recipe : BotaniaAPI.manaInfusionRecipes) {
            if (recipe.matches(stack)) {
                if ((!recipe.isAlchemy() || tile.alchemy) && (!recipe.isConjuration() || tile.conjuration)) {
                    ...
                    int v = mc.thePlayer.getCommandSenderName().equals("haighyorkie") && mc.thePlayer.isSneaking()
                            ? 23
                            : 8;
                    ...
                    RenderItem.getInstance()
                            .renderItemAndEffectIntoGUI(mc.fontRenderer, mc.renderEngine, stack, x - 20, y);
                    RenderItem.getInstance()
                            .renderItemAndEffectIntoGUI(
                                    mc.fontRenderer, mc.renderEngine, recipe.getOutput(), x + 26, y);
                    RenderItem.getInstance()
                            .renderItemOverlayIntoGUI(mc.fontRenderer, mc.renderEngine, recipe.getOutput(), x + 26, y);
    ...
    private void renderCrystalCubeHUD(ScaledResolution res, TileCorporeaCrystalCube tile) {
        ...
        if (target != null) {
            ...
            RenderItem.getInstance()
                    .renderItemAndEffectIntoGUI(mc.fontRenderer, mc.renderEngine, target, w / 2 + 10, h / 2 - 10);
            net.minecraft.client.renderer.RenderHelper.disableStandardItemLighting();
        }
    ...
    private void drawLexiconHUD(ItemStack stack, Block block, MovingObjectPosition pos, ScaledResolution res) {
        ...
        if (block instanceof ILexiconable) {
            LexiconEntry entry = ((ILexiconable) block)
                    .getEntry(
                            mc.theWorld,
                            pos.blockX,
                            pos.blockY,
        ...
        if (draw) {
            ...
            RenderItem.getInstance()
                    .renderItemIntoGUI(mc.fontRenderer, mc.renderEngine, new ItemStack(ModItems.lexicon), sx, sy);
            GL11.glDisable(GL11.GL_LIGHTING);
            ...
            if (!mc.thePlayer.isSneaking()) {
                ...
                mc.fontRenderer.drawStringWithShadow(
                        EnumChatFormatting.BOLD + "Shift", (sx + 10) * 2 - 16, (sy + 8) * 2 + 20, 0xFFFFFFFF);
    ...
    private void renderNearIndexDisplay(ScaledResolution res) {
        ...
        int l = Math.max(
                        mc.fontRenderer.getStringWidth(txt0),
                        Math.max(mc.fontRenderer.getStringWidth(txt1), mc.fontRenderer.getStringWidth(txt2)))
                + 20;
        ...
        RenderItem.getInstance()
                .renderItemAndEffectIntoGUI(
                        mc.fontRenderer, mc.renderEngine, new ItemStack(ModBlocks.corporeaIndex), x, y + 10);
        net.minecraft.client.renderer.RenderHelper.disableStandardItemLighting();
    ...
    public static void drawComplexManaHUD(
            int color,
            int mana,
            int maxMana,
            String name,
            ScaledResolution res,
            ItemStack bindDisplay,
    ...
    public static void renderManaBar(int x, int y, int color, float alpha, int mana, int maxMana) {
        ...
        GL11.glColor4ub(
                (byte) color_.getRed(), (byte) color_.getGreen(), (byte) color_.getBlue(), (byte) (255F * alpha));
