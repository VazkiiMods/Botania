...
public final class MultiblockRenderHandler {
    ...
    public static void setMultiblock(MultiblockSet set) {
        ...
        if (mc.theWorld != null) dimension = mc.theWorld.provider.dimensionId;
    }
    ...
    private void renderPlayerLook(EntityPlayer player, MovingObjectPosition src) {
        if (currentMultiblock != null && dimension == player.worldObj.provider.dimensionId) {
            ...
            rendering = true;
            Multiblock mb =
                    anchor != null ? currentMultiblock.getForIndex(angle) : currentMultiblock.getForEntity(player);
            boolean didAny = false;

            blockAccess.update(player.worldObj, mb, anchorX, anchorY, anchorZ);

            for (MultiblockComponent comp : mb.getComponents())
                if (renderComponentInWorld(player.worldObj, mb, comp, anchorX, anchorY, anchorZ)) didAny = true;
            rendering = false;
            GL11.glPopAttrib();
            GL11.glPopMatrix();
            ...
            if (!didAny) {
                setMultiblock(null);
                player.addChatComponentMessage(new ChatComponentTranslation("botaniamisc.structureComplete")
                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GREEN)));
            }
        }
    }

    private boolean renderComponentInWorld(
            World world, Multiblock mb, MultiblockComponent comp, int anchorX, int anchorY, int anchorZ) {
        ChunkCoordinates pos = comp.getRelativePosition();
        int x = pos.posX + anchorX;
        int y = pos.posY + anchorY;
        int z = pos.posZ + anchorZ;
        if (anchor != null && comp.matches(world, x, y, z)) return false;
