...
public final class RedStringRenderer {
    ...
    public static void renderAll() {
        if (!redStringTiles.isEmpty()) {
            ...
            TileRedString tile;
            while ((tile = redStringTiles.poll()) != null) renderTile(tile);
    ...
    public static void tick() {
        ...
        boolean hasWand = player != null
                && player.getCurrentEquippedItem() != null
                && player.getCurrentEquippedItem().getItem() == ModItems.twigWand;
        if (sizeAlpha > 0F && !hasWand) sizeAlpha -= 0.1F;
        else if (sizeAlpha < 1F && hasWand) sizeAlpha += 0.1F;
    }
    ...
    private static void renderTile(TileRedString tile) {
        ...
        if (bind != null) {
            GL11.glPushMatrix();
            GL11.glTranslated(
                    tile.xCoord + 0.5 - RenderManager.renderPosX,
                    tile.yCoord + 0.5 - RenderManager.renderPosY,
                    tile.zCoord + 0.5 - RenderManager.renderPosZ);
            ...
            double len = (double) -ClientTickHandler.ticksInGame / 100F
                    + new Random(dir.ordinal() ^ tile.xCoord ^ tile.yCoord ^ tile.zCoord).nextInt(10000);
    ...
    private static void addVertexAtWithTranslation(
            Tessellator tess, ForgeDirection dir, double xpos, double ypos, double zpos, double rand, double l) {
        double freq = 20;
        ...
        double z = zpos
                + (dir.offsetY == 0 ? Math.sin(l * freq) : Math.cos(l * freq))
                        * ampl
                        * Math.abs(Math.abs(dir.offsetZ) - 1)
                + rand * randMul;
