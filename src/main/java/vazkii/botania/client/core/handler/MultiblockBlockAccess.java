...
public class MultiblockBlockAccess implements IBlockAccess {
    ...
    public Block getBlock(int x, int y, int z) {
        ...
        if (comp != null) return comp.getBlock();
        if (hasBlockAccess) return originalBlockAccess.getBlock(x, y, z);
        return Blocks.air;
    }
    ...
    public TileEntity getTileEntity(int x, int y, int z) {
        MultiblockComponent comp = getComponent(x, y, z);
        if (comp != null) return comp.getTileEntity();
        if (hasBlockAccess) return originalBlockAccess.getTileEntity(x, y, z);
        return null;
    }

    @Override
    public int getLightBrightnessForSkyBlocks(int x, int y, int z, int p_72802_4_) {
        if (hasBlockAccess) return originalBlockAccess.getLightBrightnessForSkyBlocks(x, y, z, p_72802_4_);
        return 15728640;
    }
    ...
    public int getBlockMetadata(int x, int y, int z) {
        MultiblockComponent comp = getComponent(x, y, z);
        if (comp != null) return comp.getMeta();
        if (hasBlockAccess) return originalBlockAccess.getBlockMetadata(x, y, z);
        return 0;
    }
    ...
    public boolean isAirBlock(int x, int y, int z) {
        ...
        if (hasBlockAccess) return originalBlockAccess.isAirBlock(x, y, z);
        return true;
    }

    @Override
    public BiomeGenBase getBiomeGenForCoords(int x, int z) {
        if (hasBlockAccess) return originalBlockAccess.getBiomeGenForCoords(x, z);
        return null;
    }

    @Override
    public int getHeight() {
        if (hasBlockAccess) return originalBlockAccess.getHeight();
        return 256;
    }

    @Override
    public boolean extendedLevelsInChunkCache() {
        if (hasBlockAccess) return originalBlockAccess.extendedLevelsInChunkCache();
        return false;
    }

    @Override
    public boolean isSideSolid(int x, int y, int z, ForgeDirection side, boolean _default) {
        if (hasBlockAccess) return originalBlockAccess.isSideSolid(x, y, z, side, _default);
        return _default;
    }
