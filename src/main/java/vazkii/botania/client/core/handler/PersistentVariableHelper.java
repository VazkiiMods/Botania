...
public final class PersistentVariableHelper {
    ...
    public static void save() throws IOException {
        ...
        for (Challenge c : ModChallenges.challengeLookup.values()) c.writeToNBT(challengesCmp);
        ...
        for (String s : GuiLexicon.notes.keySet()) {
            ...
            if (note != null && !note.trim().isEmpty()) notesCmp.setString(s, note);
        }
    ...
    public static void load() throws IOException {
        ...
        if (cmp.hasKey(TAG_CHALLENGES)) {
            ...
            for (Challenge c : ModChallenges.challengeLookup.values()) c.readFromNBT(challengesCmp);
        }
        ...
        if (cmp.hasKey(TAG_LEXICON_NOTES)) {
            ...
            for (String key : keys) GuiLexicon.notes.put(key, notesCmp.getString(key));
        }
        ...
        if (firstLoad) lastBotaniaVersion = LibMisc.VERSION;
