...
public final class ItemsRemainingRenderHandler {
    ...
    public static void render(ScaledResolution resolution, float partTicks) {
        if (ticks > 0 && stack != null) {
            ...
            if (count >= 0) {
                ...
                if (stacks == 0) text = "" + count;
                else
                    text = count + " (" + EnumChatFormatting.AQUA + stacks + EnumChatFormatting.RESET + "*"
                            + EnumChatFormatting.GRAY + max + EnumChatFormatting.RESET + "+" + EnumChatFormatting.YELLOW
                            + rem + EnumChatFormatting.RESET + ")";
    ...
    public static void set(EntityPlayer player, ItemStack displayStack, Pattern pattern) {
        ...
        for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
            ...
            if (stack != null && pattern.matcher(stack.getUnlocalizedName()).find()) count += stack.stackSize;
        }
