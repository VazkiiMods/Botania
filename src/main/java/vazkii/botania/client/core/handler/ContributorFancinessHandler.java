...
public final class ContributorFancinessHandler {
    ...
    public static void render(RenderPlayerEvent.Specials event) {
        ...
        if (name.equals("Vazkii") || name.equals("_phi")) {
            if (phi) renderPhiFlower(event);
            else renderTwintails(event);
        } else if (name.equals("haighyorkie")) renderGoldfish(event);

        firstStart();

        name = name.toLowerCase();
        if (Minecraft.getMinecraft().gameSettings.getOptionOrdinalValue(Options.SHOW_CAPE)
                && flowerMap != null
                && flowerMap.containsKey(name)) renderFlower(event, flowerMap.get(name));
    }
    ...
    public static void load(Properties props) {
        ...
        for (String key : props.stringPropertyNames()) {
            ...
            try {
                ...
                if (i < 0 || i >= 16) throw new NumberFormatException();
    ...
    private static void renderTwintails(RenderPlayerEvent event) {
        ...
        if (event.entityPlayer.motionY < 0) GL11.glRotatef((float) event.entityPlayer.motionY * 20F, 1F, 0F, 0F);
    ...
    public static class ThreadContributorListLoader extends Thread {
        ...
        public void run() {
            try {
                ...
                FMLLog.info(
                        "[Botania] Could not load contributors list. Either you're offline or github is down. Nothing to worry about, carry on~");
                e.printStackTrace();
            }
        }
    }
