...
public final class BossBarHandler {
    ...
    public static void render(ScaledResolution res) {
        ...
        if (e.isDead
                || !p.worldObj.loadedEntityList.contains(e)
                || MathHelper.pointDistanceSpace(e.posX, e.posY, e.posZ, p.posX, p.posY, p.posZ) > 32)
            currentBoss = null;
    }
    ...
    public static void drawBar(int x, int y, int u, int v, int w, int h, boolean bg) {
        ...
        if (useShader) ShaderHelper.releaseShader();
    }
