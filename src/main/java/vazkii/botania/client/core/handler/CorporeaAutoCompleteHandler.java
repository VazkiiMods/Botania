...
public class CorporeaAutoCompleteHandler {
            ...
            if (item != null && item.getCreativeTab() != null) {
                ...
                try {
                    ...
                    for (ItemStack stack : curList)
                        itemNames.add(CorporeaHelper.stripControlCodes(
                                stack.getDisplayName().trim()));
                } catch (Exception e) {
                }
            }
        }
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if (isAutoCompleted) {
            ...
            if (!valid) isAutoCompleted = false;
        }
        if (Keyboard.isKeyDown(15)) {
            if (tabLastTick) return;
            tabLastTick = true;
        } else {
            tabLastTick = false;
            return;
        ...
        if (!CorporeaHelper.shouldAutoComplete()) return;

        GuiTextField inputField = ReflectionHelper.getPrivateValue(GuiChat.class, chat, LibObfuscation.INPUT_FIELD);
        if (!isAutoCompleted) buildAutoCompletes(inputField, chat);
        if (isAutoCompleted && !completions.isEmpty()) advanceAutoComplete(inputField, chat);
    }

    private void advanceAutoComplete(GuiTextField inputField, GuiChat chat) {
        position++;
        if (position >= completions.size()) position -= completions.size();
    ...
    private void buildAutoCompletes(GuiTextField inputField, GuiChat chat) {
        String leftOfCursor;
        if (inputField.getCursorPosition() == 0) leftOfCursor = "";
        else leftOfCursor = inputField.getText().substring(0, inputField.getCursorPosition());
        if (leftOfCursor.length() == 0 || leftOfCursor.charAt(0) == '/') return;
        ...
        CompletionData data;
        for (Iterator<CompletionData> iterator = completions.iterator();
                iterator.hasNext();
    ...
    private ArrayList<CompletionData> getNames(String prefix) {
        ...
        while (i >= 0) {
            ...
            i--;
            if (i >= 0) curPrefix = words[i] + " " + curPrefix;
        }
    ...
    private List<CompletionData> getNamesStartingWith(String prefix) {
        ...
        for (String str : after) {
            if (str.toLowerCase().startsWith(prefix)) result.add(new CompletionData(str, length));
            else return result;
        }
        return result;
    }
