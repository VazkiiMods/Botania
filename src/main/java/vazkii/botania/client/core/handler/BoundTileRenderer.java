...
public final class BoundTileRenderer {
    ...
    public void onWorldRenderLast(RenderWorldLastEvent event) {
        ...
        if (stack != null && stack.getItem() instanceof ICoordBoundItem) {
            ...
            if (coords != null) renderBlockOutlineAt(coords, color);
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (stackInSlot != null && stackInSlot.getItem() instanceof IWireframeCoordinateListProvider) {
                ...
                if (coordsList != null) for (ChunkCoordinates coords : coordsList) renderBlockOutlineAt(coords, color);

                if (stackInSlot.getItem() instanceof IExtendedWireframeCoordinateListProvider) {
                    ChunkCoordinates coords = ((IExtendedWireframeCoordinateListProvider) stackInSlot.getItem())
                            .getSourceWireframe(player, stackInSlot);
                    if (coords != null && coords.posY > -1) renderBlockOutlineAt(coords, color, 5F);
                }
            }
        }
    ...
    private void renderBlockOutlineAt(ChunkCoordinates pos, int color, float thickness) {
        ...
        GL11.glTranslated(
                ...
                pos.posZ - RenderManager.renderPosZ + 1);
        ...
        drawWireframe:
        {
            if (block != null) {
                ...
                GL11.glColor4ub(
                        (byte) colorRGB.getRed(), (byte) colorRGB.getGreen(), (byte) colorRGB.getBlue(), (byte) 64);
                renderBlockOutline(axis);
            }
        }
