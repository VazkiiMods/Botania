...
public interface PaintslingerLensPaintBlockCallback {
    Event<PaintslingerLensPaintBlockCallback> EVENT = EventFactory.createArrayBacked(PaintslingerLensPaintBlockCallback.class,
            listeners -> (player, pos, color, originalBlock, newBlock, lens) -> {
                for (PaintslingerLensPaintBlockCallback listener : listeners) {
                    if (listener.onPaintslingerLensPaintBlock(player, pos, color, originalBlock, newBlock, lens)) {
                        return true;
                    }
                }
                return false;
    ...
    boolean onPaintslingerLensPaintBlock(Player player, BlockPos pos, int color, Block originalBlock, Block newBlock, ItemStack lens);
}